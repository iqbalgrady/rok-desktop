...
const STREAM_PERF_STORAGE_KEY = 'rok-desktop_stream_perf';
...
declare global {
    interface Window {
        __rokDesktopStreamPerfState?: StreamPerfState;
        __rokDesktopVsCodeStreamPerfState?: {
            counters: Map<string, PerfCounter>;
            lastReportAt?: number;
            lastUpdatedAt?: number;
...
const ensureStreamPerfState = (): StreamPerfState | null => {
    ...
    if (!window.__rokDesktopStreamPerfState) {
        const startedAt = Date.now();
        window.__rokDesktopStreamPerfState = {
    ...
    return window.__rokDesktopStreamPerfState;
};
...
export const setStreamPerfEnabled = (enabled: boolean): void => {
    ...
    try {
        ...
        delete window.__rokDesktopStreamPerfState;
        delete window.__rokDesktopVsCodeStreamPerfState;
    } catch {
...
export const resetStreamPerf = (): void => {
    ...
    if (window.__rokDesktopVsCodeStreamPerfState) {
        window.__rokDesktopVsCodeStreamPerfState = {
            ...window.__rokDesktopVsCodeStreamPerfState,
            counters: new Map<string, PerfCounter>(),
            startedAt: Date.now(),
            lastUpdatedAt: Date.now(),
...
export const getStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__rokDesktopStreamPerfState;
...
export const getVsCodeStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__rokDesktopVsCodeStreamPerfState;
    if (!streamPerfEnabled() || !state) {
        return {
            enabled: false,
            startedAt: null,
