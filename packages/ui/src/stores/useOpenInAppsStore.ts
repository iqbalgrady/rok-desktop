...
export const useOpenInAppsStore = create<OpenInAppsState>()((set, get) => ({
  ...
  initialize: () => {
    ...
    window.addEventListener('rok-desktop:settings-synced', settingsHandler);
    window.addEventListener('rok-desktop:app-ready', appReadyHandler);
    window.addEventListener('rok-desktop:installed-apps-updated', updateHandler);

    const appReady = (window as unknown as { __rokDesktopAppReady?: boolean }).__rokDesktopAppReady;
    if (appReady) {
      void loadInstalledApps();
    }
