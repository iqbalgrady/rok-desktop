...
import type { Provider, Agent, Config } from "@/lib/rokcode/types";
...
const MODELS_DEV_PROXY_URL = "/api/rok-desktop/models-metadata";
...
const fetchRokDesktopDefaults = async (): Promise<RokDesktopDefaults> => {
    markStartupTrace('config.defaults:start');
    const started = typeof performance !== 'undefined' ? performance.now() : Date.now();
    const finish = (source: string, result: RokDesktopDefaults) => {
...
interface ConfigStore {
    ...
    // Rok Desktop settings-based defaults (take precedence over agent preferences)
            ...
            (set, get) => ({
                ...
                sttProvider: (() => {
                    if (typeof window !== 'undefined') {
                        ...
                        const electron = (window as unknown as { __ROK_DESKTOP_ELECTRON__?: { runtime?: string } }).__ROK_DESKTOP_ELECTRON__;
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and Rok Desktop settings in parallel. OpenCode config
                            ...
                            // Detect invalid Rok Desktop settings so we can clear them from storage.
                ...
                setAgent: (agentName: string | undefined) => {
                    ...
                    if (agentName) {
                        ...
                        if (currentSessionId && useSessionUIStore.getState().isRokDesktopCreatedSession(currentSessionId)) {
                            const existingAgentModel = selState.getAgentModelForSession(currentSessionId, agentName);
                            if (!existingAgentModel) {
                                useSessionUIStore.getState().initializeNewRokDesktopSession(currentSessionId, agents);
                            }
                        }
                    }
