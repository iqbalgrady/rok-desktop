...
import { getWorktreeSetupWaitEnabled, saveWorktreeSetupCommands } from '@/lib/rokDesktopConfig';
...
const registerCreatedSession = (session: Session, directory: string): Session => {
  ...
  useSessionUIStore.getState().markSessionAsRokDesktopCreated(session.id);
