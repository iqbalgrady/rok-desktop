...
const MENU_ACTION_EVENT = 'rok-desktop:menu-action';
const CHECK_FOR_UPDATES_EVENT = 'rok-desktop:check-for-updates';
...
export const useMenuActions = (
  ...
  const handleAction = React.useCallback(
    (action: MenuAction) => {
      switch (action) {
        ...
        case 'copy': {
          const copyEvent = new Event('rok-desktop:copy', { cancelable: true });
        ...
        case 'download-logs': {
          void showOpenCodeStatus().catch(() => {
            toast.error('Failed to collect Rokcode status');
          });
          break;
        }
      }
  ...
  React.useEffect(() => {
    ...
    const desktop = (window as unknown as { __ROK_DESKTOP_DESKTOP__?: DesktopBridgeGlobal }).__ROK_DESKTOP_DESKTOP__;
    ...
    listen('rok-desktop:menu-action', (evt) => {
    ...
    listen('rok-desktop:check-for-updates', () => {
