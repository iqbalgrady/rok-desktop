...
const TRAY_ACTION_EVENT = 'rok-desktop:tray-action';
...
type TraySnapshot = {
  ...
  // Active instance label (e.g. "Local Rok Desktop" or a remote host name) so
...
// the existing `rok-desktop:open-session` / `rok-desktop:open-draft-session`
...
const isMac = (): boolean => {
  ...
  return (window as unknown as { __ROK_DESKTOP_PLATFORM__?: string }).__ROK_DESKTOP_PLATFORM__ === 'darwin';
};
...
// the local origin shows as "Local Rok Desktop"; a remote host shows its
// configured name. Async because the host config is read over IPC.
const resolveInstanceName = async (): Promise<string> => {
  try {
    if (isDesktopLocalOriginActive()) return 'Local Rok Desktop';
    const localOrigin = (window as unknown as { __ROK_DESKTOP_LOCAL_ORIGIN__?: string }).__ROK_DESKTOP_LOCAL_ORIGIN__
      || window.location.origin;
    const runtimeApiBaseUrl = getRuntimeApiBaseUrl();
    if (runtimeApiBaseUrl && locationMatchesHost(runtimeApiBaseUrl, localOrigin)) return 'Local Rok Desktop';
...
export const useTraySync = (): void => {
  ...
  React.useEffect(() => {
    ...
    const bridge = (window as unknown as { __ROK_DESKTOP_DESKTOP__?: DesktopBridgeGlobal }).__ROK_DESKTOP_DESKTOP__;
