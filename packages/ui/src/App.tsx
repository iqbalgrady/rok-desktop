...
function App({ apis }: AppProps) {
  React.useEffect(() => {
    ...
    if (startupTraceEnabled()) {
      console.info('[startup-trace] enabled. Run console.table(window.__ROK_DESKTOP_STARTUP_TRACE__) after startup.');
    }
  }, []);
  ...
  React.useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'rok-desktop:embedded-visibility') {
        return;
      }
    ...
    const scopedWindow = window as unknown as {
      __rokDesktopSetEmbeddedVisibility?: (payload?: EmbeddedVisibilityPayload) => void;
    };

    scopedWindow.__rokDesktopSetEmbeddedVisibility = applyVisibility;
    window.addEventListener('message', handleMessage);

    return () => {
      window.removeEventListener('message', handleMessage);
      if (scopedWindow.__rokDesktopSetEmbeddedVisibility === applyVisibility) {
        delete scopedWindow.__rokDesktopSetEmbeddedVisibility;
      }
    };
  }, [embeddedSessionChat]);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rok-desktop:open-session', handler as EventListener);
    return () => window.removeEventListener('rok-desktop:open-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rok-desktop:open-mini-chat', onOpenMiniChat);
    return () => window.removeEventListener('rok-desktop:open-mini-chat', onOpenMiniChat);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rok-desktop:open-draft-session', handler as EventListener);
    return () => window.removeEventListener('rok-desktop:open-draft-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rok-desktop:open-project', handler as EventListener);
    return () => window.removeEventListener('rok-desktop:open-project', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    (window as unknown as { __rokDesktopAppReady?: boolean }).__rokDesktopAppReady = true;
    window.dispatchEvent(new Event('rok-desktop:app-ready'));
  ...
  // The Rust backend sets window.__ROK_DESKTOP_DESKTOP_BOOT_OUTCOME__ once the
