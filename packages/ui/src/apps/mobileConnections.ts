...
const MOBILE_CONNECTIONS_STORAGE_KEY = 'rok-desktop.mobile.connections.v1';
const MOBILE_SECURE_STORAGE_PREFIX = 'rok-desktop.mobile.';
  ...
  const submitPassword = React.useCallback(async (password: string) => {
    ...
    try {
      ...
      const response = await requestWithTimeout(`${url}/auth/session`, {
        ...
        body: JSON.stringify({ password, trustDevice: true, issueClientToken: true, clientLabel: 'Rok Desktop Mobile' }),
      });
