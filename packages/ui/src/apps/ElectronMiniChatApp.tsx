...
const MINI_CHAT_PRESENCE_CHANNEL = 'rok-desktop:mini-chat-presence';
...
const MiniChatBootstrap: React.FC<{ config: MiniChatConfig }> = ({ config }) => {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rok-desktop:open-session', onOpenSession);
    return () => window.removeEventListener('rok-desktop:open-session', onOpenSession);
