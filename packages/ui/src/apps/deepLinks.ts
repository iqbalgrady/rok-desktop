/**
 * Rok Desktop deep-link vocabulary — the single source of truth for the `rok-desktop://`
...
export const DEEP_LINK_SCHEME = 'rok-desktop';
...
const segmentsOf = (url: URL): string[] => {
  // Custom-scheme URLs put the first route token in `host` (rok-desktop://session/<id>),
  // but be tolerant of authority-less forms (rok-desktop:/session/<id>) where it lands in
...
/**
 * Parse a raw `rok-desktop://…` string into a typed intent, or `null` if it isn't a
 * recognised Rok Desktop deep link. Tolerant by design: unknown routes return `null`
...
/**
 * Build a canonical `rok-desktop://…` URL for an intent. Used by anything that needs to hand
