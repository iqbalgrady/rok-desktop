...
describe('createRuntimeUrlResolver', () => {
  ...
  test('uses realtime base URL for SSE and WebSocket URLs', () => {
    ...
    expect(urls.sse('/api/rok-desktop/events')).toBe('https://realtime.example/api/rok-desktop/events');
  ...
  test('uses injected desktop API base URL for packaged WebSocket URLs', () => {
    withWindow({
      location: { origin: 'rok-desktop-ui://app', href: 'rok-desktop-ui://app/index.html' },
      __ROK_DESKTOP_API_BASE_URL__: 'http://127.0.0.1:57123',
    }, () => {
      const urls = createRuntimeUrlResolver({});
  ...
  test('routes realtime URLs through local desktop proxy when runtime headers are configured', () => {
    ...
    try {
      withWindow({
        location: { origin: 'rok-desktop-ui://app', href: 'rok-desktop-ui://app/index.html' },
        __ROK_DESKTOP_API_BASE_URL__: 'https://remote.example',
        __ROK_DESKTOP_LOCAL_ORIGIN__: 'http://127.0.0.1:57123',
      }, () => {
        const urls = createRuntimeUrlResolver({});
        ...
        expect(sse.pathname).toBe('/api/rok-desktop/realtime-proxy/sse');
        expect(sse.searchParams.get('url')).toBe('https://remote.example/api/global/event');
        expect(ws.origin).toBe('ws://127.0.0.1:57123');
        expect(ws.pathname).toBe('/api/rok-desktop/realtime-proxy/ws');
  ...
  test('reads injected desktop API base URL at call time', () => {
    withWindow({
      ...
      const urls = createRuntimeUrlResolver({});
      (window as typeof window & { __ROK_DESKTOP_API_BASE_URL__?: string }).__ROK_DESKTOP_API_BASE_URL__ = 'http://127.0.0.1:57123';
  ...
  test('adds short-lived URL auth query to realtime and authenticated asset URLs only', () => {
    ...
    try {
      ...
      expect(urls.sse('/api/rok-desktop/events')).toBe(
        'https://api.example/api/rok-desktop/events?oc_url_token=oc_url_secret',
      );
      expect(urls.websocket('/api/global/event/ws', { lastEventId: 'evt-1' })).toBe(
  ...
  test('does not put the long-lived client token in URLs', () => {
    ...
    try {
      const urls = createRuntimeUrlResolver({ apiBaseUrl: 'https://api.example' });
      expect(urls.sse('/api/rok-desktop/events')).toBe('https://api.example/api/rok-desktop/events');
