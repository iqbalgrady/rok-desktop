...
declare global {
  interface Window {
    __ROK_DESKTOP_STARTUP_TRACE__?: StartupTraceEvent[];
    __ROK_DESKTOP_STARTUP_TRACE_START__?: number;
  }
}
...
export const markStartupTrace = (name: string, data?: Record<string, unknown>) => {
  ...
  window.__ROK_DESKTOP_STARTUP_TRACE_START__ ??= now;
  window.__ROK_DESKTOP_STARTUP_TRACE__ ??= [];
  window.__ROK_DESKTOP_STARTUP_TRACE__.push({
    t: Math.round(now - window.__ROK_DESKTOP_STARTUP_TRACE_START__),
    name,
    ...(data ? { data } : {}),
  });
  if (window.__ROK_DESKTOP_STARTUP_TRACE__.length > MAX_STARTUP_TRACE_EVENTS) {
    window.__ROK_DESKTOP_STARTUP_TRACE__.splice(
      0,
      window.__ROK_DESKTOP_STARTUP_TRACE__.length - MAX_STARTUP_TRACE_EVENTS,
    );
  }
};

const getStartupTraceSummary = () => {
  const trace = typeof window !== 'undefined' ? window.__ROK_DESKTOP_STARTUP_TRACE__ ?? [] : [];
...
if (typeof window !== 'undefined') {
  (window as typeof window & { __ROK_DESKTOP_STARTUP_TRACE_SUMMARY__?: typeof getStartupTraceSummary })
    .__ROK_DESKTOP_STARTUP_TRACE_SUMMARY__ = getStartupTraceSummary;
}
