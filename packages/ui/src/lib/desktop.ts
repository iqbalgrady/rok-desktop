...
export type DesktopSettings = {
  ...
  showRokcodeUpdateNotifications?: boolean;
  ...
  // User-added skills catalogs (persisted to ~/.config/rok-desktop/settings.json)
  skillCatalogs?: SkillCatalogConfig[];
  // Opt-in to send anonymous usage reports for update checks (default: true)
  reportUsage?: boolean;

  // Global behavior prompt — synced to ~/.config/rokcode/AGENTS.md
...
const getElectronRuntime = (): ElectronRuntimeGlobal | null => {
  ...
  return (window as unknown as { __ROK_DESKTOP_ELECTRON__?: ElectronRuntimeGlobal }).__ROK_DESKTOP_ELECTRON__ ?? null;
};

const getDesktopBridge = (): DesktopBridgeGlobal | null => {
  if (typeof window === 'undefined') return null;
  return (window as unknown as { __ROK_DESKTOP_DESKTOP__?: DesktopBridgeGlobal }).__ROK_DESKTOP_DESKTOP__ ?? null;
};
...
export const isDesktopLocalOriginActive = (): boolean => {
  ...
  const local = typeof window.__ROK_DESKTOP_LOCAL_ORIGIN__ === 'string' ? window.__ROK_DESKTOP_LOCAL_ORIGIN__ : '';
...
export const getDesktopHomeDirectory = async (): Promise<string | null> => {
  if (typeof window !== 'undefined') {
    const embedded = window.__ROK_DESKTOP_HOME__;
...
export const downloadDesktopUpdate = async (
  ...
  try {
    if (typeof onProgress === 'function' && bridge?.listen) {
      unlisten = await bridge.listen('rok-desktop:update-progress', (evt) => {
