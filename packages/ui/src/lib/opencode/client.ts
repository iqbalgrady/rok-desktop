...
const createRuntimeRokcodeClient = (config: { baseUrl: string; directory?: string }): RokcodeClient => {
  return createRokcodeClient({
    ...config,
    fetch: runtimeFetch,
  });
};
...
class OpencodeService {
  private client: RokcodeClient;
  private baseUrl: string;
  private scopedClients: Map<string, RokcodeClient> = new Map();
  ...
  constructor(baseUrl: string = DEFAULT_BASE_URL) {
    ...
    this.client = createRuntimeRokcodeClient({ baseUrl: this.baseUrl });
  }
  ...
  getSdkClient(): RokcodeClient {
    return this.client;
  }

  /** Get a scoped SDK client for a specific directory */
  getScopedSdkClient(directory: string): RokcodeClient {
   ...
   */
  getScopedApiClient(directory: string): RokcodeClient {
    ...
    const scoped = createRuntimeRokcodeClient({ baseUrl: this.baseUrl, directory: normalized });
...
');
  ...
  async setOpenCodeWorkingDirectory(directoryPath: string | null | undefined): Promise<DirectorySwitchResult | null> {
    if (!directoryPath || typeof directoryPath !== 'string' || !directoryPath.trim()) {
      console.warn('[RokcodeClient] setOpenCodeWorkingDirectory: invalid path', directoryPath);
      return null;
    }

    const url = `${this.baseUrl}/opencode/directory`;
    console.log('[RokcodeClient] POST', url, 'with path:', directoryPath);
