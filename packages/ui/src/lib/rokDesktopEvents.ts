...
type RokDesktopEvent = ScheduledTaskRanEvent;
type Listener = (event: RokDesktopEvent) => void;
...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'rok-desktop:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type === 'rok-desktop:heartbeat') {
    return;
  }

  if (envelope.type !== 'rok-desktop:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const source = new EventSource(getRuntimeUrlResolver().sse('/api/rok-desktop/events'));
  source.onopen = () => {
    resetHeartbeatTimer();
  };
...
export const subscribeRokDesktopEvents = (listener: Listener): (() => void) => {
