...
import rokDesktopLightRaw from './fields-of-the-shire-light.json';
import rokDesktopDarkRaw from './fields-of-the-shire-dark.json';

const flexokiLightTheme = withPrColors(flexokiLightRaw as Theme);
const flexokiDarkTheme = withPrColors(flexokiDarkRaw as Theme);
const rokDesktopLightTheme = withPrColors(rokDesktopLightRaw as Theme);
const rokDesktopDarkTheme = withPrColors(rokDesktopDarkRaw as Theme);

export const DEFAULT_LIGHT_THEME_ID = 'flexoki-light' as const;
export const DEFAULT_DARK_THEME_ID = 'flexoki-dark' as const;

export const themes: Theme[] = [
  rokDesktopLightTheme,
  rokDesktopDarkTheme,
  flexokiLightTheme,
  flexokiDarkTheme,
  ...presetThemes.filter(
    (theme) => theme.metadata.id !== 'rok-desktop-light' && theme.metadata.id !== 'rok-desktop-dark',
  ),
];
