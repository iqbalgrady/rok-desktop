...
type RokDesktopOpencodeResolution = {
  configured?: unknown;
  resolved?: unknown;
  resolvedDir?: unknown;
  source?: unknown;
...
const buildOpenCodeStatusReport = async (): Promise<string> => {
  ...
  const rokDesktopHealth: RokDesktopHealthSnapshot | null = await (async () => {
    ...
    try {
      ...
      return json as RokDesktopHealthSnapshot;
    } catch {
      return null;
    } finally {
      clearTimeout(timeout);
  ...
  const openChamberOpencodeResolutionResult: {
    data: RokDesktopOpencodeResolution | null;
    status: number | null;
    error: string | null;
  } = await (async () => {
    ...
    try {
      ...
      return { data: json as RokDesktopOpencodeResolution, status: resp.status, error: null };
    } catch (error) {
      return {
        data: null,
        status: null,
  ...
  if (typeof window !== 'undefined') {
    const injected = (window as unknown as { __ROK_DESKTOP_MACOS_MAJOR__?: unknown }).__ROK_DESKTOP_MACOS_MAJOR__;
  ...
  if (isLikelyMac) {
    ...
    const resolved =
      openChamberOpencodeResolution && typeof openChamberOpencodeResolution.resolved === 'string'
        ...
        : (rokDesktopHealth && typeof rokDesktopHealth.opencodeBinaryResolved === 'string' ? rokDesktopHealth.opencodeBinaryResolved : '');
    const resolvedDir =
    ...
    const source =
      openChamberOpencodeResolution && typeof openChamberOpencodeResolution.source === 'string'
        ...
        : (rokDesktopHealth && typeof rokDesktopHealth.opencodeBinarySource === 'string' ? rokDesktopHealth.opencodeBinarySource : '');
    const configuredLaunchBinary =
      openChamberOpencodeResolution && typeof openChamberOpencodeResolution.launchBinary === 'string'
        ...
        : (rokDesktopHealth && typeof rokDesktopHealth.opencodeLaunchBinary === 'string' ? rokDesktopHealth.opencodeLaunchBinary : '');
    const configuredLaunchWrapperType =
      openChamberOpencodeResolution && typeof openChamberOpencodeResolution.launchWrapperType === 'string'
        ...
        : (rokDesktopHealth && typeof rokDesktopHealth.opencodeLaunchWrapperType === 'string' ? rokDesktopHealth.opencodeLaunchWrapperType : '');
    const configuredLaunchArgs =
      openChamberOpencodeResolution && Array.isArray(openChamberOpencodeResolution.launchArgs)
        ...
        : (rokDesktopHealth && Array.isArray(rokDesktopHealth.opencodeLaunchArgs)
          ? rokDesktopHealth.opencodeLaunchArgs.filter((value): value is string => typeof value === 'string')
          : []);
    const node =
      openChamberOpencodeResolution && typeof openChamberOpencodeResolution.node === 'string'
        ? openChamberOpencodeResolution.node
        : (rokDesktopHealth && typeof rokDesktopHealth.nodeBinaryResolved === 'string' ? rokDesktopHealth.nodeBinaryResolved : '');
    const bun =
      openChamberOpencodeResolution && typeof openChamberOpencodeResolution.bun === 'string'
        ? openChamberOpencodeResolution.bun
        : (rokDesktopHealth && typeof rokDesktopHealth.bunBinaryResolved === 'string' ? rokDesktopHealth.bunBinaryResolved : '');
    const detectedNow =
