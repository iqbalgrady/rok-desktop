...
const RUNTIME_ENDPOINT_CHANGED_EVENT = 'rok-desktop:runtime-endpoint-changed';

let activeApiBaseUrl = '';
let activeRuntimeKey = '';

const setWindowRuntimeValue = <K extends '__ROK_DESKTOP_API_BASE_URL__' | '__ROK_DESKTOP_CLIENT_TOKEN__' | '__ROK_DESKTOP_RUNTIME_HEADERS__'>(
...
const readInjectedApiBaseUrl = (): string => {
  ...
  const injected = (window as typeof window & { __ROK_DESKTOP_API_BASE_URL__?: string }).__ROK_DESKTOP_API_BASE_URL__;
  return typeof injected === 'string' ? injected.trim() : '';
};

const readInjectedLocalOrigin = (): string => {
  if (typeof window === 'undefined') return '';
  const injected = (window as typeof window & { __ROK_DESKTOP_LOCAL_ORIGIN__?: string }).__ROK_DESKTOP_LOCAL_ORIGIN__;
  return typeof injected === 'string' ? injected.trim() : '';
};
...
export const switchRuntimeEndpoint = (options: { apiBaseUrl: string; clientToken?: string | null; runtimeKey?: string | null; requestHeaders?: Record<string, string> | null }): void => {
  ...
  if (typeof window !== 'undefined') {
    ...
    setWindowRuntimeValue(runtimeWindow, '__ROK_DESKTOP_API_BASE_URL__', apiBaseUrl);
    setWindowRuntimeValue(runtimeWindow, '__ROK_DESKTOP_CLIENT_TOKEN__', options.clientToken || undefined);
    setWindowRuntimeValue(runtimeWindow, '__ROK_DESKTOP_RUNTIME_HEADERS__', options.requestHeaders || undefined);
  }
