...
const readInjectedBearerToken = (): string => {
  ...
  const injected = (window as typeof window & { __ROK_DESKTOP_CLIENT_TOKEN__?: string }).__ROK_DESKTOP_CLIENT_TOKEN__;
...
export const getRuntimeExtraHeadersSync = (): Record<string, string> => {
  ...
  if (typeof window === 'undefined') return {};
  const injected = (window as typeof window & { __ROK_DESKTOP_RUNTIME_HEADERS__?: Record<string, string> }).__ROK_DESKTOP_RUNTIME_HEADERS__;
