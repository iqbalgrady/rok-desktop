...
export const applyPersistedHomeDirectoryToWindow = (homeDirectory: string): void => {
  ...
  if (typeof window.__ROK_DESKTOP_HOME__ === 'string' && window.__ROK_DESKTOP_HOME__.length > 0) {
    return;
  }

  try {
    window.__ROK_DESKTOP_HOME__ = homeDirectory;
  } catch {
...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (typeof settings.pwaAppName === 'string') {
    ...
    if (normalized.length > 0) {
      localStorage.setItem('rok-desktop.pwaName', normalized);
    } else {
      localStorage.removeItem('rok-desktop.pwaName');
    }
  }
...
const dispatchSettingsSynced = (settings: DesktopSettings): void => {
  ...
  window.dispatchEvent(new CustomEvent<DesktopSettings>('rok-desktop:settings-synced', { detail: settings }));
};
