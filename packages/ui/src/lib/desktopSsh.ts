...
export type DesktopSshInstance = {
  ...
  auth: {
    ...
    rokDesktopPassword?: DesktopSshStoredSecret;
  };
...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const rokDesktopPassword = parseStoredSecret(authRaw.rokDesktopPassword || authRaw.rok_desktop_password);
  ...
  return {
    ...
    auth: {
      ...
      ...(rokDesktopPassword ? { rokDesktopPassword } : {}),
    },
    portForwards,
  };
};
...
export const listenDesktopSshStatus = async (
  ...
  const unlisten = await listen('rok-desktop:ssh-instance-status', (event) => {
