/**
 * Rok Desktop project-level configuration service.
 * Stores per-project settings in ~/.config/rok-desktop/<projectId>.json.
 * Migrates from legacy <project>/.rokcode/rok-desktop.json.
 */
...
interface RokDesktopConfig {
  projectPath?: string;
  'setup-worktree'?: string[];
  'setup-worktree-wait'?: boolean;
  projectNotes?: string;
  projectTodos?: RokDesktopProjectTodoItem[];
  projectPlanFiles?: RokDesktopProjectPlanFileLink[];
  projectActions?: RokDesktopProjectAction[];
  projectActionsPrimaryId?: string;
  draftStarters?: DraftStarterRef[];
}

type RokDesktopProjectActionPlatform = 'macos' | 'linux' | 'windows';

export interface RokDesktopProjectAction {
  id: string;
  name: string;
  command: string;
  icon?: string | null;
  platforms?: RokDesktopProjectActionPlatform[];
  autoOpenUrl?: boolean;
  openUrl?: string;
  desktopOpenSshForward?: string;
}

export interface RokDesktopProjectActionsState {
  actions: RokDesktopProjectAction[];
  primaryActionId: string | null;
}

export interface RokDesktopProjectTodoItem {
  id: string;
  text: string;
  completed: boolean;
  createdAt: number;
...
export interface RokDesktopProjectPlanFileLink {
  id: string;
  path: string;
  createdAt: number;
}

export interface RokDesktopProjectPlanFile {
  title: string;
  body: string;
  raw: string;
  path: string;
...
export interface RokDesktopProjectNotesTodos {
  notes: string;
  todos: RokDesktopProjectTodoItem[];
}

export interface RokDesktopProjectContextData extends RokDesktopProjectNotesTodos {
  plans: RokDesktopProjectPlanFileLink[];
}

export const ROK_DESKTOP_PROJECT_NOTES_MAX_LENGTH = 3000;
export const ROK_DESKTOP_PROJECT_TODO_TEXT_MAX_LENGTH = 120;
const ROK_DESKTOP_PROJECT_ACTION_NAME_MAX_LENGTH = 80;
const ROK_DESKTOP_PROJECT_ACTION_COMMAND_MAX_LENGTH = 4000;
const ROK_DESKTOP_PROJECT_ACTION_OPEN_URL_MAX_LENGTH = 2000;
const ROK_DESKTOP_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH = 300;
const OPENCHAMBER_PROJECT_PLAN_TITLE_MAX_LENGTH = 160;

const OPENCHAMBER_ACTION_PLATFORM_SET = new Set<RokDesktopProjectActionPlatform>(['macos', 'linux', 'windows']);
...
const resolveHomeDirectory = async (): Promise<string | null> => {
  ...
  // In some runtimes, window.__ROK_DESKTOP_HOME__ can be workspace/project-root
...
const sanitizeProjectNotes = (value: unknown): string => {
  ...
  return trimToMaxLength(value, ROK_DESKTOP_PROJECT_NOTES_MAX_LENGTH);
};

const sanitizeProjectTodoItems = (value: unknown): RokDesktopProjectTodoItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: RokDesktopProjectTodoItem[] = [];
  for (const entry of value) {
    ...
    const text = trimToMaxLength(textRaw.trim(), ROK_DESKTOP_PROJECT_TODO_TEXT_MAX_LENGTH);
    if (!id || !text) {
      continue;
    }
...
const sanitizeProjectPlanFileLinks = (value: unknown): RokDesktopProjectPlanFileLink[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: RokDesktopProjectPlanFileLink[] = [];
...
const sanitizeProjectActionPlatforms = (value: unknown): RokDesktopProjectActionPlatform[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const unique: RokDesktopProjectActionPlatform[] = [];
  const seen = new Set<RokDesktopProjectActionPlatform>();
  for (const entry of value) {
    if (typeof entry !== 'string') {
      continue;
    }
    const normalized = entry.trim().toLowerCase() as RokDesktopProjectActionPlatform;
...
const sanitizeProjectActions = (value: unknown): RokDesktopProjectAction[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: RokDesktopProjectAction[] = [];
  const seenIds = new Set<string>();
  ...
  for (const entry of value) {
    ...
    const id = typeof record.id === 'string' ? record.id.trim() : '';
    const name = trimToMaxLength(typeof record.name === 'string' ? record.name.trim() : '', ROK_DESKTOP_PROJECT_ACTION_NAME_MAX_LENGTH);
    const command = trimToMaxLength(typeof record.command === 'string' ? record.command.trim() : '', ROK_DESKTOP_PROJECT_ACTION_COMMAND_MAX_LENGTH);
    ...
    const openUrl = trimToMaxLength(openUrlRaw, ROK_DESKTOP_PROJECT_ACTION_OPEN_URL_MAX_LENGTH);
    ...
    const desktopOpenSshForward = trimToMaxLength(
      ...
      ROK_DESKTOP_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH
    );
...
} | null | undefined): RokDesktopProjectActionsState => {
...
const sanitizeProjectNotesAndTodos = (value: {
  notes?: unknown;
  todos?: unknown;
} | null | undefined): RokDesktopProjectNotesTodos => {
  return {
...
const sanitizeProjectContextData = (value: {
  ...
  todos?: unknown;
  plans?: unknown;
} | null | undefined): RokDesktopProjectContextData => {
 ...
 */
async function readRokDesktopConfig(project: ProjectRef): Promise<RokDesktopConfig | null> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return null;
  }
  ...
  const parseConfig = (text: string | null): RokDesktopConfig | null => {
    ...
    try {
      ...
      return parsed as RokDesktopConfig;
    } catch {
      return null;
    }
  };
  ...
  // 2) Migrate legacy <project>/.rokcode/rok-desktop.json.
  // LEGACY_PROJECT_CONFIG: migrate project-local legacy.json -> ~/.config/rok-desktop/projects/<projectId>.json
  ...
  try {
    const wrote = await writeRokDesktopConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyRokDesktopConfig(projectDirectory);
    }
  } catch {
 ...
 */
async function writeRokDesktopConfig(
  project: ProjectRef,
  config: RokDesktopConfig
): Promise<boolean> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return false;
  ...
  try {
    ...
    console.error('Failed to write rok-desktop config:', error);
    return false;
  }
}
 ...
 */
async function updateRokDesktopConfig(
  project: ProjectRef,
  updates: Partial<RokDesktopConfig>
): Promise<boolean> {
  const existing = await readRokDesktopConfig(project) || {};
  const merged = { ...existing, ...updates };
  return writeRokDesktopConfig(project, merged);
}
...
export async function getWorktreeSetupCommands(project: ProjectRef): Promise<string[]> {
  const config = await readRokDesktopConfig(project);
...
export async function saveWorktreeSetupCommands(project: ProjectRef, commands: string[]): Promise<boolean> {
  ...
  return updateRokDesktopConfig(project, { 'setup-worktree': filtered });
}
...
export async function saveWorktreeSetupWaitEnabled(project: ProjectRef, enabled: boolean): Promise<boolean> {
  return updateRokDesktopConfig(project, { 'setup-worktree-wait': enabled });
}
...
export async function saveProjectDraftStarters(project: ProjectRef, starters: DraftStarterRef[]): Promise<boolean> {
  return updateRokDesktopConfig(project, { draftStarters: sanitizeStarterRefs(starters) });
}

export async function getProjectNotesAndTodos(project: ProjectRef): Promise<RokDesktopProjectNotesTodos> {
  const config = await readRokDesktopConfig(project);
...
export async function saveProjectNotesAndTodos(
  project: ProjectRef,
  value: RokDesktopProjectNotesTodos
): Promise<boolean> {
  ...
  return updateRokDesktopConfig(project, {
    projectNotes: sanitized.notes,
    projectTodos: sanitized.todos,
  });
}

export async function getProjectContextData(project: ProjectRef): Promise<RokDesktopProjectContextData> {
  const config = await readRokDesktopConfig(project);
...
async function getProjectPlanFiles(project: ProjectRef): Promise<RokDesktopProjectPlanFileLink[]> {
  const config = await readRokDesktopConfig(project);
...
async function saveProjectPlanFiles(
  project: ProjectRef,
  value: RokDesktopProjectPlanFileLink[]
): Promise<boolean> {
...
export async function readProjectPlanFile(path: string): Promise<RokDesktopProjectPlanFile | null> {
...
): Promise<RokDesktopProjectPlanFileLink | null> {
...
export async function getProjectActionsState(project: ProjectRef): Promise<RokDesktopProjectActionsState> {
  const config = await readRokDesktopConfig(project);
...
export async function saveProjectActionsState(
  project: ProjectRef,
  value: RokDesktopProjectActionsState
): Promise<boolean> {
...
async function deleteLegacyRokDesktopConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();
