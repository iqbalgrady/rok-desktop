...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __ROK_DESKTOP_VSCODE_THEME__?: Theme }).__ROK_DESKTOP_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('rok-desktop:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('rok-desktop:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  const updateBrowserChrome = useCallback((theme: Theme) => {
    ...
    const hasMacVibrancy = document.documentElement.hasAttribute('data-oc-vibrancy')
      || window.__ROK_DESKTOP_ELECTRON__?.macVibrancy === true;
  ...
  useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __rokDesktopApplyThemeSync?: (payload: ThemeSyncPayload) => void;
    };

    scopedWindow.__rokDesktopApplyThemeSync = applyIncomingThemeSync;

    return () => {
      if (scopedWindow.__rokDesktopApplyThemeSync === applyIncomingThemeSync) {
        delete scopedWindow.__rokDesktopApplyThemeSync;
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'rok-desktop:theme-sync' || !data.payload) {
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('rok-desktop:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('rok-desktop:settings-synced', handleSettingsSynced);
  }, []);
