...
import type { Session, Part, Message, TextPart } from "@/lib/rokcode/types"
...
import { getWorktreeSetupWaitEnabled } from "@/lib/rokDesktopConfig"
...
export type SessionUIState = {
  ...
  clearError: () => void
  markSessionAsRokDesktopCreated: (sessionId: string) => void
  isRokDesktopCreatedSession: (sessionId: string) => boolean
  getContextUsage: (contextLimit: number, outputLimit: number) => SessionContextUsage | null
  initializeNewRokDesktopSession: (sessionId: string, agents: unknown[]) => void
...
export async function materializeOpenDraftSession(selection: {
  ...
  store.initializeNewRokDesktopSession(created.id, configState.agents ?? [])
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  markSessionAsRokDesktopCreated: (sessionId) =>
    set((s) => {
  ...
  isRokDesktopCreatedSession: (sessionId) => get().webUICreatedSessions.has(sessionId),
  ...
  initializeNewRokDesktopSession: () => {
