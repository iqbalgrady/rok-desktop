...
import type { Event, OpencodeClient, SessionStatus } from "@/lib/rokcode/types"
...
const normalizeRokDesktopSessionStatus = (payload: Event): Event | null => {
  ...
  if (record.type !== "rok-desktop:session-status") return null
  ...
  return {
    id: typeof record.id === "string" && record.id.length > 0
      ? record.id
      : `rok-desktop-status-${sessionID}-${Date.now()}`,
    type: "session.status",
    properties: {
      sessionID,
      status,
...
const normalizeEventType = (payload: Event): Event => {
  const normalizedRokDesktopStatus = normalizeRokDesktopSessionStatus(payload)
  if (normalizedRokDesktopStatus) {
    return normalizedRokDesktopStatus
  }
...
export function createEventPipeline(input: EventPipelineInput): EventPipeline {
  ...
  if (typeof globalThis.window !== "undefined") {
    globalThis.window.addEventListener("rok-desktop:system-resume", onSystemResume)
  ...
  const cleanup = () => {
    ...
    if (typeof globalThis.window !== "undefined") {
      globalThis.window.removeEventListener("rok-desktop:system-resume", onSystemResume)
