...
import type { OpencodeClient } from "@/lib/rokcode/types"
...
import type { SessionStatus } from "@/lib/rokcode/types"
...
const SYNC_CONTEXT_GLOBAL_KEY = "__rokDesktop_sync_context__"
...
const handleUiNotificationEvent = (payload: Event, fallbackDirectory: string): boolean => {
  if ((payload as { type?: unknown }).type !== "rok-desktop:notification") {
    return false
  }
...
const dispatchVSCodeRuntimeNotificationEvent = (directory: string, payload: Event) => {
  ...
  window.dispatchEvent(new CustomEvent("rok-desktop:vscode-notification-event", {
...
const dispatchRokcodeUpdateAvailable = (payload: { version: string }) => {
  ...
  window.dispatchEvent(new CustomEvent("rok-desktop:rokcode-update-available", { detail: payload }))
}
