  ...
  const handleSaveAndReload = React.useCallback(async () => {
    ...
    try {
      ...
      await reloadOpenCodeConfiguration({
        message: t('settings.rok-desktop.opencodeCli.actions.restartingOpenCode'),
        mode: 'projects',
        scopes: ['all'],
      });
    } finally {
      ...
      <div className="mb-1 px-1">
        <div className="flex items-center gap-2">
          <h3 className="typography-ui-header font-medium text-foreground">
            {t('settings.rok-desktop.opencodeCli.title')}
          </h3>
          <Tooltip>
            ...
            <TooltipContent sideOffset={8} className="max-w-xs">
              {t('settings.rok-desktop.opencodeCli.tooltipPrefix')}
              {' '}
              <code className="font-mono text-xs">opencode</code>
              {t('settings.rok-desktop.opencodeCli.tooltipSuffix')}
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
      ...
      <section className="px-2 pb-2 pt-0 space-y-0.5">
        <div data-settings-item="sessions.opencode-binary" className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-3">
          <div className="flex min-w-0 flex-col shrink-0">
            <span className="typography-ui-label text-foreground">{t('settings.rok-desktop.opencodeCli.field.binaryPath')}</span>
          </div>
          <div className="flex min-w-0 items-center gap-2 sm:w-[20rem]">
            <Input
              value={value}
              onChange={(e) => setValue(e.target.value)}
              placeholder={t('settings.rok-desktop.opencodeCli.field.binaryPathPlaceholder')}
            ...
            <Button
              ...
              aria-label={t('settings.rok-desktop.opencodeCli.actions.browseAria')}
              title={t('settings.rok-desktop.opencodeCli.actions.browse')}
            >
        ...
        <div className="py-1.5">
          <div className="typography-micro text-muted-foreground/70">
            ...
            {' '}
            <span className="font-mono">~/.config/rok-desktop/settings.json</span>
            {'.'}
          </div>
        </div>
        ...
        <label data-settings-item="sessions.rokcode-update-notifications" className="flex cursor-pointer items-center gap-2 py-1.5">
          ...
          <span className="typography-ui-label text-foreground">
            {t('settings.rok-desktop.opencodeCli.field.showUpdateNotifications')}
          </span>
        </label>
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.rok-desktop.opencodeCli.actions.saveAndReload')}
          </Button>
        </div>
      </section>
    </div>
