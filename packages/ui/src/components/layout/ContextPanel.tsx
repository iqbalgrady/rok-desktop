...
const DESKTOP_BROWSER_INSPECT_SCRIPT = `new Promise((resolve) => {
  const existing = document.getElementById('__rokDesktop_desktop_browser_overlay');
  if (existing) existing.remove();
  if (typeof window.__rokDesktopDesktopBrowserCancelInspect === 'function') {
    try { window.__rokDesktopDesktopBrowserCancelInspect(); } catch { /* webview not ready */ }
  }
  const overlay = document.createElement('div');
  overlay.id = '__rokDesktop_desktop_browser_overlay';
  ...
  const cleanup = () => {
    ...
    if (window.__rokDesktopDesktopBrowserCancelInspect === cancel) {
      delete window.__rokDesktopDesktopBrowserCancelInspect;
    }
  };
  const cancel = () => {
    cleanup();
  ...
  window.__rokDesktopDesktopBrowserCancelInspect = cancel;
...
const DESKTOP_BROWSER_CANCEL_INSPECT_SCRIPT = `(() => {
  if (typeof window.__rokDesktopDesktopBrowserCancelInspect === 'function') {
    window.__rokDesktopDesktopBrowserCancelInspect();
    return;
  }
  const overlay = document.getElementById('__rokDesktop_desktop_browser_overlay');
  if (overlay) overlay.remove();
})()`;

const DESKTOP_BROWSER_SAME_WEBVIEW_NAVIGATION_SCRIPT = `(() => {
  if (window.__rokDesktopSameWebviewNavigationInstalled) return;
  window.__rokDesktopSameWebviewNavigationInstalled = true;
...
const PreviewPane: React.FC<PreviewPaneProps> = ({ rawUrl, onNavigate }) => {
  ...
  React.useEffect(() => {
    ...
    postPreviewBridgeMessage(frameWindow, proxySrc, {
      source: 'rok-desktop-preview-parent',
      version: 1,
  ...
  React.useEffect(() => {
    ...
    const handler = (event: MessageEvent<PreviewBridgeMessage>) => {
      ...
      if (!data || data.source !== 'rok-desktop-preview-bridge' || data.version !== 1) {
        return;
      }
...
const isElectronBrowserRuntime = (): boolean => {
  return typeof window !== 'undefined' && Boolean(window.__ROK_DESKTOP_ELECTRON__);
};
...
const IframeBrowserPane: React.FC<DesktopBrowserPaneProps> = ({ initialUrl, directory, tabID }) => {
  ...
  React.useEffect(() => {
    const handler = (event: MessageEvent<PreviewBridgeMessage>) => {
      ...
      const data = event.data;
      if (!data || data.source !== 'rok-desktop-preview-bridge' || data.version !== 1) return;
      ...
      <div className="relative min-h-0 flex-1 bg-background">
        {iframeSrc ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 bg-background p-6 text-center">
            <RokDesktopLogo width={140} height={140} className="opacity-20" />
    ...
    <div className="absolute inset-0 flex flex-col bg-background">
      ...
      <div className="relative min-h-0 flex-1 bg-background">
        <webview
          ...
          partition="persist:rok-desktop-browser"
          allowpopups
...
export const ContextPanel: React.FC = () => {
  ...
  const postThemeSyncToEmbeddedChat = React.useCallback(() => {
    ...
    for (const frame of chatFrameRefs.current.values()) {
      ...
      const directThemeSync = (frameWindow as unknown as {
        __rokDesktopApplyThemeSync?: (themePayload: typeof payload) => void;
      }).__rokDesktopApplyThemeSync;
  ...
  const postEmbeddedVisibilityToChats = React.useCallback(() => {
    ...
    for (const [tabID, frame] of chatFrameRefs.current.entries()) {
      ...
      const directVisibilitySync = (frameWindow as unknown as {
        __rokDesktopSetEmbeddedVisibility?: (visibilityPayload: typeof payload) => void;
      }).__rokDesktopSetEmbeddedVisibility;
      ...
      frameWindow.postMessage(
        {
          type: 'rok-desktop:embedded-visibility',
          payload,
        },
        window.location.origin,
      );
  ...
  React.useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'rok-desktop:cycle-theme-request') {
        return;
      }
