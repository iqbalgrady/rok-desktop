...
export const Header: React.FC<HeaderProps> = ({
  ...
  const isWindowsElectronDesktop = React.useMemo(() => {
    ...
    return Boolean(window.__ROK_DESKTOP_ELECTRON__) && window.__ROK_DESKTOP_PLATFORM__ === 'win32';
  }, []);
  ...
  const refreshCurrentInstanceLabel = React.useCallback(async () => {
    ...
    try {
      ...
      const localOrigin = window.__ROK_DESKTOP_LOCAL_ORIGIN__ || window.location.origin;
  ...
  const checkRemoteInstanceUpdate = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await runtimeFetch(`/api/rok-desktop/update-check?${params.toString()}`, {
        method: 'GET',
  ...
  const currentSessionTitle = React.useMemo(() => {
    if (!currentSessionId) {
      return activeProjectLabel ?? 'Rok Desktop';
    }
  ...
  useEffect(() => {
    ...
    void syncFullscreenState();
    window.addEventListener('rok-desktop:window-resized', onResize);

    return () => {
      disposed = true;
      window.removeEventListener('rok-desktop:window-resized', onResize);
    };
