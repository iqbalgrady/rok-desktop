...
const AUTO_DISCOVER_ACTION_ID = '__rokDesktop_auto_discover_preview__';
...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<RokDesktopProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<RokDesktopProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: RokDesktopProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: RokDesktopProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<RokDesktopProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: RokDesktopProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: RokDesktopProjectAction, toggleStopIfRunning = false) => {
