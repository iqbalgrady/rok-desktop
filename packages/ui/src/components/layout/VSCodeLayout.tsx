...
export const VSCodeLayout: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () => (typeof window !== 'undefined'
      ? (window as { __ROK_DESKTOP_CONNECTION__?: { status?: string } }).__ROK_DESKTOP_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('rok-desktop.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('rok-desktop.updateSessionEditorTitle', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as { __ROK_DESKTOP_CONNECTION__?: { status?: string } }).__ROK_DESKTOP_CONNECTION__?.status
    ...
    window.addEventListener('rok-desktop:connection-status', handler as EventListener);
    return () => window.removeEventListener('rok-desktop:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rok-desktop:navigate', handler as EventListener);
    return () => window.removeEventListener('rok-desktop:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('rok-desktop_stream_debug') === '1';
          } catch {
            return false;
          }
        })();

        if (debugEnabled) console.log('[Rok Desktop][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[Rok Desktop][VSCode][bootstrap] post-load', {
