...
export const WindowsWindowControls = React.memo(function WindowsWindowControls({ visible }: WindowsWindowControlsProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('rok-desktop:window-maximized-changed', handleMaximizedChange);
    return () => {
      disposed = true;
      window.removeEventListener('rok-desktop:window-maximized-changed', handleMaximizedChange);
    };
  }, [visible]);
