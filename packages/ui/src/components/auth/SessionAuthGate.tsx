...
const TRUST_DEVICE_STORAGE_KEY = 'rok-desktop.uiAuth.trustDevice';
...
const issueDesktopClientToken = async (): Promise<string> => {
  ...
  const response = await runtimeFetch('/api/client-auth/clients', {
    ...
    body: JSON.stringify({ label: 'Rok Desktop', ...desktopClientAuthMetadata() }),
  }).catch(() => null);
  if (!response?.ok) {
    return '';
  }
...
const LoadingScreen: React.FC = () => (
  <div className="flex min-h-screen items-center justify-center bg-background text-foreground">
    <RokDesktopLogo width={120} height={120} />
  </div>
);
