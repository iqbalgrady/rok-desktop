...
import type { Part } from '@/lib/rokcode/types';
...
const useExternalLinkInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (anchor.getAttribute('data-rok-desktop-file-link') === 'true') {
        return;
      }
...
const FILE_LINK_SELECTOR = '[data-rok-desktop-file-link="true"]';
const BLOCK_PATH_TOKEN_ATTR = 'data-rok-desktop-block-path-token';
const BLOCK_PATH_TOKEN_SELECTOR = `[${BLOCK_PATH_TOKEN_ATTR}]`;
const CODE_BLOCK_PATH_SCANNED_ATTR = 'data-rok-desktop-block-paths-scanned';
...
// `data-rok-desktop-block-path-token`. `annotateFileLinks` then promotes those
...
// `data-rok-desktop-block-paths-scanned` once processed so the walk is not
...
const useFileReferenceInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const clearFileLinkAttributes = (candidate: HTMLElement) => {
      candidate.removeAttribute('data-rok-desktop-file-link');
      candidate.removeAttribute('data-rok-desktop-file-ref');
      candidate.removeAttribute('data-rok-desktop-file-path');
    ...
    const annotateFileLinks = () => {
      ...
      for (const candidate of Array.from(candidates)) {
        ...
        void existsPromise.then((exists) => {
          ...
          candidate.setAttribute('data-rok-desktop-file-link', 'true');
          candidate.setAttribute('data-rok-desktop-file-ref', latestRawCandidate);
          candidate.setAttribute('data-rok-desktop-file-path', latestResolved.resolvedPath);
    ...
    const openFileReference = async (sourceElement: HTMLElement) => {
      const raw = sourceElement.getAttribute('data-rok-desktop-file-ref') || extractPathCandidateFromElement(sourceElement);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target;
      if (!(target instanceof HTMLElement) || target.getAttribute('data-rok-desktop-file-link') !== 'true') {
        return;
      }
