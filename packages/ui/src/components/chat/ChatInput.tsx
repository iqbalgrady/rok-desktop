...
const getDraftKey = (sessionId: string | null): string =>
    `rok-desktop_chat_input_draft_${sessionId ?? 'new'}`;
...
const getConfirmedMentionsKey = (sessionId: string | null): string =>
    `rok-desktop_chat_confirmed_mentions_${sessionId ?? 'new'}`;
...
const ChatInputComponent: React.FC<ChatInputProps> = ({ onOpenSettings, scrollToBottom }) => {
    ...
    const [agentMode, setAgentMode] = React.useState<string | undefined>(undefined);
    ...
    const handleCodingToolSelect = React.useCallback((slashCommand: string) => {
        setMessage(slashCommand + " ");
        if (textareaRef.current) {
            textareaRef.current.focus();
        }
    }, [setMessage]);
    ...
    const hasDraggedFiles = React.useCallback((dataTransfer: DataTransfer | null | undefined): boolean => {
        ...
        if (dataTransfer.types) {
            ...
            if (lowerTypes.includes('application/x-rok-desktop-file-path')) return true;
    ...
    const handleDragEnter = (e: React.DragEvent) => {
        ...
        const isInternal = e.dataTransfer.types?.includes('application/x-rok-desktop-file-path') ?? false;
    ...
    const handleDrop = async (e: React.DragEvent) => {
        ...
        const internalPath = e.dataTransfer.getData('application/x-rok-desktop-file-path');
                            ...
                            <>
                                <div className={cn("flex items-center flex-shrink-0", footerGapClass)}>
                                    ...
                                    <CodingTools
                                        footerIconButtonClass={footerIconButtonClass}
                                        iconSizeClass={iconSizeClass}
                                        onToolSelect={handleCodingToolSelect}
                                    />
                                    <ModeSelector
                                        footerIconButtonClass={footerIconButtonClass}
                                        iconSizeClass={iconSizeClass}
