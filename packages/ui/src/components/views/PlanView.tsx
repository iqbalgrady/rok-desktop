...
import { parseProjectPlanMarkdown } from '@/lib/rokDesktopConfig';
...
export const PlanView: React.FC<PlanViewProps> = ({ targetPath = null }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'rok-desktop:plan:md-viewer-mode';
  ...
  const handleConfirmPlanSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingPlanSend.target === 'worktree') {
          ...
          initializeNewRokDesktopSession(sessionResult.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, currentProjectRef, initializeNewRokDesktopSession, pendingPlanSend, resolvedPath, routeToChat, sendMessage, sendPromptTitle, setCurrentSession]
  );
