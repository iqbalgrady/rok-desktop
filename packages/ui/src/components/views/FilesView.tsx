...
const FILE_EDITOR_AUTO_SAVE_KEY = 'rok-desktop:files:auto-save-enabled';
type FileLineEnding = '\n' | '\r\n';
...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'rok-desktop:files:md-viewer-mode';
  const HTML_VIEWER_MODE_KEY = 'rok-desktop:files:html-viewer-mode';
  const JSON_VIEWER_MODE_KEY = 'rok-desktop:files:json-viewer-mode';
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rok-desktop:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    return () => {
      window.removeEventListener('rok-desktop:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    };
  }, [openPaths]);
