...
import type { RokDesktopSection } from '@/components/sections/rok-desktop/types';
import { RokDesktopPage } from '@/components/sections/rok-desktop/RokDesktopPage';
...
const SETTINGS_DETAIL_HISTORY_KEY = '__rokDesktopSettingsDetail';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed, visiblePageSlugs, initialMobileStage = 'nav' }) => {
  ...
  const openChamberSectionBySlug: Partial<Record<SettingsPageSlug, RokDesktopSection>> = React.useMemo(() => ({
    appearance: 'visual',
    chat: 'chat',
    shortcuts: 'shortcuts',
    sessions: 'sessions',
  ...
  const getPageTitle = React.useCallback((slug: SettingsPageSlug): string => {
    ...
    case 'rokcode':
      return t('settings.page.rokcode.title');
      case 'about':
  ...
  const openSearchResult = React.useCallback((result: SettingsSearchResult) => {
    ...
    if (result.id === 'plugins.create' && typeof window !== 'undefined') {
      window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('rok-desktop:settings-open-plugin-add'));
      }, 50);
    }
  ...
  const renderPageContent = React.useCallback((slug: SettingsPageSlug) => {
    ...
    switch (slug) {
      ...
      case 'rokcode': {
        ...
        return <RokDesktopPage section={section} />;
      }
      default:
        return <SettingsHome onOpen={openPage} />;
    }
