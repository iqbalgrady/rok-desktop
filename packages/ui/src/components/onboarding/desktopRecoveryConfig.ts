...
export function getDesktopRecoveryConfig(
  ...
  switch (variant) {
    case 'local-unavailable':
      return {
        title: 'Local Rokcode Unavailable',
        description: 'Rokcode CLI could not be started or is not installed. Install Rokcode or connect to a remote server instead.',
    ...
    case 'remote-wrong-service': {
      ...
      return {
        ...
        description: `The server at "${host || 'unknown'}" is not running Rok Desktop. Verify the address points to an Rok Desktop server.`,
    ...
    case 'remote-incompatible': {
      ...
      return {
        ...
        description: `The Rok Desktop server at "${host || 'unknown'}" is not compatible with this app version. Update Rok Desktop on the server, then try again.`,
