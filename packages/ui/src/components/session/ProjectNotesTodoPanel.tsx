...
type ProjectPlanListItem = RokDesktopProjectPlanFileLink & {
  title: string;
};

const toPlanListItem = async (
  plan: RokDesktopProjectPlanFileLink,
...
const sortTodosWithCompletedLast = (items: RokDesktopProjectTodoItem[]): RokDesktopProjectTodoItem[] => [
  ...items.filter((todo) => !todo.completed),
  ...items.filter((todo) => todo.completed),
];

const insertTodoBeforeCompleted = (items: RokDesktopProjectTodoItem[], item: RokDesktopProjectTodoItem): RokDesktopProjectTodoItem[] => {
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const [todos, setTodos] = React.useState<RokDesktopProjectTodoItem[]>([]);
  ...
  const initializeNewRokDesktopSession = useSessionUIStore((state) => state.initializeNewRokDesktopSession);
  ...
  const persistProjectData = React.useCallback(
    async (nextNotes: string, nextTodos: RokDesktopProjectTodoItem[]) => {
      if (!projectRef) {
        return false;
      }
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rok-desktop:project-plan-saved', handleProjectContextRefresh);
    window.addEventListener('rok-desktop:project-notes-updated', handleProjectContextRefresh);
    return () => {
      window.removeEventListener('rok-desktop:project-plan-saved', handleProjectContextRefresh);
      window.removeEventListener('rok-desktop:project-notes-updated', handleProjectContextRefresh);
    };
  }, [projectRef]);
  ...
  const handleConfirmSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingSendTarget.kind === 'worktree') {
          ...
          initializeNewRokDesktopSession(session.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, initializeNewRokDesktopSession, onActionComplete, pendingSendTarget, projectRef, routeToChat, sendMessage, setCurrentSession, t]
  );
  ...
  const handleDeletePlan = React.useCallback(
    async (planId: string) => {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent('rok-desktop:project-plan-saved', {
