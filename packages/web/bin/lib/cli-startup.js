...
const STARTUP_SERVICE_ID = 'dev.rok-desktop.web';
...
function getStartupServicePaths() {
  ...
  if (process.platform === 'linux') {
    return {
      platform: 'linux',
      servicePath: path.join(os.homedir(), '.config', 'systemd', 'user', 'rok-desktop.service'),
    };
  }
...
function getMacosStartupWrapperPath() {
  return path.join(getDataDir(), 'bin', 'Rok Desktop');
}
...
function collectStartupEnv(options = {}) {
  ...
  if (uiPassword) {
    env.ROK_DESKTOP_UI_PASSWORD = uiPassword;
  }
  if (options.apiOnly === true) {
    env.ROK_DESKTOP_API_ONLY = 'true';
  }
  if (typeof process.env.ROK_DESKTOP_DATA_DIR === 'string' && process.env.ROK_DESKTOP_DATA_DIR.trim().length > 0) {
    env.ROK_DESKTOP_DATA_DIR = path.resolve(process.env.ROK_DESKTOP_DATA_DIR.trim());
  }
  return env;
}
...
function buildMacosLaunchAgent(options = {}) {
  ...
  const env = collectStartupEnv(options);
  const logDir = path.join(os.homedir(), 'Library', 'Logs', 'Rok Desktop');
...
function buildSystemdUserService(options = {}) {
  ...
  const envFilePath = getStartupEnvFilePath();
  return `[Unit]
Description=Rok Desktop web server
...
function getStartupStatus() {
  ...
  if (paths.platform === 'linux') {
    const enabledResult = runStartupCommand('systemctl', ['--user', 'is-enabled', 'rok-desktop.service'], { allowFailure: true });
    const activeResult = runStartupCommand('systemctl', ['--user', 'is-active', 'rok-desktop.service'], { allowFailure: true });
...
function enableStartupService(options = {}) {
  ...
  if (paths.platform === 'macos') {
    ...
    fs.mkdirSync(path.join(os.homedir(), 'Library', 'Logs', 'Rok Desktop'), { recursive: true, mode: 0o700 });
  ...
  if (paths.platform === 'linux') {
    ...
    runStartupCommand('systemctl', ['--user', 'enable', '--now', 'rok-desktop.service']);
    return getStartupStatus();
  }
...
function disableStartupService() {
  ...
  if (paths.platform === 'linux') {
    runStartupCommand('systemctl', ['--user', 'disable', '--now', 'rok-desktop.service'], { allowFailure: true });
    try { fs.unlinkSync(paths.servicePath); } catch {}
