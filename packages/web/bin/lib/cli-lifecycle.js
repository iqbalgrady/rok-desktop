...
function createLivePortInstance(port, info, host) {
  ...
  return {
    ...
    pidFilePath: path.join(getRunDir(), `rok-desktop-${port}.pid`),
    instanceFilePath: path.join(getRunDir(), `rok-desktop-${port}.json`),
    mtime: 0,
    startedAt: 0,
    launchMode: 'daemon',
...
async function resolveDoctorPortStatuses(options = {}) {
  ...
  if (options.explicitPort) {
    ...
    if (desktopEntry && desktopEntry.port === requestedPort) {
      statuses.push({
        ...
        detail: 'Use a CLI instance port from `rok-desktop serve` for tunneling.',
      });
    ...
    statuses.push({
      ...
      detail: `Start one with \`rok-desktop serve --port ${requestedPort}\`.`,
    });
    return { statuses, availableEntries: [] };
  }
  ...
  if (runningEntries.length === 0) {
    statuses.push({
      ...
      detail: 'Start one with `rok-desktop serve`.',
    });
  }
...
async function discoverRunningInstances(options = {}) {
  ...
  try {
    ...
    const pidFiles = files.filter((file) => file.startsWith('rok-desktop-') && file.endsWith('.pid'));
    for (const file of pidFiles) {
      const port = parseInt(file.replace('rok-desktop-', '').replace('.pid', ''), 10);
      if (!Number.isFinite(port) || port <= 0) continue;
      const pidFilePath = path.join(runDir, file);
      const pid = readPidFile(pidFilePath);
      if (!pid) {
        removePidFile(pidFilePath);
        removeInstanceFile(path.join(runDir, `rok-desktop-${port}.json`));
        continue;
      }

      const instanceFilePath = path.join(runDir, `rok-desktop-${port}.json`);
      ...
      // confirms Rok Desktop. Cmdline identity alone can match a recycled PID
      // from another Rok Desktop process on a different port. Try all plausible
...
async function discoverRokDesktopInstanceOnPort(port, options = {}) {
...
async function discoverLifecycleInstances(options = {}, deps = {}) {
  ...
  const liveInstance = await discoverRokDesktopInstanceOnPort(options.port, {
    ...deps,
    host: options.host,
    runningInstances,
  });
