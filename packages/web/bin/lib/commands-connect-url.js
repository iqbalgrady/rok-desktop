...
async function resolveConnectUrlServerUrl(options) {
  ...
  if (typeof hostOverride !== 'string' && !process.env.ROK_DESKTOP_HOST) {
...
function getRokDesktopDataDir() {
  return process.env.ROK_DESKTOP_DATA_DIR
...
function buildClientConnectionPayload({ serverUrl, token, label }) {
  ...
  return `rok-desktop://connect?${params.toString()}`;
}
...
function createConnectUrlCommand({ serveCommand }) {
  return async function connectUrlCommand(options = {}) {
    assertSafeBrowserPort(options.port, { context: 'Rok Desktop connect-url' });
    ...
    const label = options.name || `Rok Desktop ${serverUrl}`;
    const runtime = createRemoteClientAuthRuntime({
      fsPromises: fs.promises,
      path,
      crypto,
      storePath: path.join(getRokDesktopDataDir(), REMOTE_CLIENTS_FILE_NAME),
    });
    ...
    clackIntro('Rok Desktop connect URL');
    if (serverState.autoStarted) {
      logStatus('success', `started Rok Desktop on port ${options.port}`);
    }
    logStatus('success', connectUrl);
    clackLog.info(`Server URL: ${serverUrl}`);
    if (resolvedServerUrl.source === 'lan-detected') {
      clackLog.info('Detected a LAN address because Rok Desktop is bound to all interfaces. Use --server to override it.');
    } else if (resolvedServerUrl.source === 'loopback-fallback') {
      clackLog.warn('Rok Desktop is bound to all interfaces, but no LAN address was detected. Use --server to provide a reachable URL.');
    }
    clackLog.info('Copy this connection link into another Rok Desktop client. The token is shown only once.');
    if (options.qr === true) {
