...
async function startupCommand(options, action = 'status') {
  ...
  if (!['status', 'enable', 'disable'].includes(normalized)) {
    throw new TunnelCliError(
      `Unknown startup subcommand '${action}'. Use 'rok-desktop startup --help'.`,
  ...
  if (normalized === 'enable' && result.activeState === 'failed') {
    throw new TunnelCliError(
      'Startup service was installed but failed to start. Run `journalctl --user -u rok-desktop.service -n 80 --no-pager` for details.',
  ...
  clackIntro('Rok Desktop Startup');
  ...
  if (normalized === 'enable') {
    logStatus('info', 'service command', 'rok-desktop serve --foreground');
  }
