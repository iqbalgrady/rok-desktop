...
import { discoverRokDesktopInstanceOnPort, isDesktopRuntimeForPort } from './cli-lifecycle.js';
...
async function serveCommand(options) {
    ...
    if (targetPort !== 0 && !options.suppressUnsafePortWarning) {
      assertSafeBrowserPort(targetPort, { context: 'Rok Desktop serve' });
    }

    if (targetPort !== 0) {
      const existingInstance = await discoverRokDesktopInstanceOnPort(targetPort, { host: effectiveHost });
      if (existingInstance?.runtime === 'desktop') {
        throw new Error(
          `Port ${targetPort} is used by Rok Desktop Desktop app. Choose another port or stop the desktop app.`
        );
      }
      if (existingInstance) {
        const pidSuffix = Number.isFinite(existingInstance.pid) ? ` (PID: ${existingInstance.pid})` : '';
        if (existingInstance.source === 'probe') {
          throw new Error(`Rok Desktop is already running on port ${targetPort}. Use \`rok-desktop status\` or \`rok-desktop stop --port ${targetPort}\`.`);
        }
        throw new Error(`Rok Desktop is already running on port ${targetPort}${pidSuffix}`);
      }
    ...
    if (!effectiveUiPassword && !options.suppressUiPasswordWarning) {
      ...
      const warningLine = 'ROK_DESKTOP_UI_PASSWORD is not set';
      const warningDetail = networkExposed
        ...
        : 'browser UI is unsecured. Use --ui-password or ROK_DESKTOP_UI_PASSWORD.';
      if (showOutput) {
    ...
    if (options.foreground) {
      ...
      if (effectiveUiPassword) {
        process.env.ROK_DESKTOP_UI_PASSWORD = effectiveUiPassword;
      }
      process.env.ROK_DESKTOP_HOST = effectiveHost;
      process.env.ROK_DESKTOP_RUNTIME = 'web';
      ...
      if (!isQuietMode(options)) {
        console.log(`Starting Rok Desktop on port ${targetPort === 0 ? 'auto' : targetPort} (foreground)`);
      }
    ...
    const child = spawn(runtimeBin, serverArgs, {
      ...
      env: {
        ...process.env,
        ROK_DESKTOP_PORT: String(targetPort),
        ROK_DESKTOP_RUNTIME: 'web',
        OPENCODE_BINARY: opencodeBinary,
        ROK_DESKTOP_HOST: effectiveHost,
        ...(effectiveUiPassword ? { ROK_DESKTOP_UI_PASSWORD: effectiveUiPassword } : {}),
        ...(options.apiOnly === true ? { ROK_DESKTOP_API_ONLY: 'true' } : {}),
        ...(process.env.OPENCODE_SKIP_START ? { ROK_DESKTOP_SKIP_OPENCODE_START: process.env.OPENCODE_SKIP_START } : {}),
      },
    });

    child.unref();
    serveSpin?.start(`Starting Rok Desktop on port ${targetPort === 0 ? 'auto' : targetPort}...`);
    ...
    try {
      resolvedPort = await new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          if (settled) return;
          settled = true;
          reject(new Error(`Rok Desktop daemon did not report ready within ${DAEMON_READY_TIMEOUT_MS / 1000}s`));
        }, DAEMON_READY_TIMEOUT_MS);

        child.on('message', (msg) => {
          if (settled) return;
          if (msg && msg.type === 'rok-desktop:ready' && typeof msg.port === 'number') {
            settled = true;
        ...
        child.on('exit', (code, signal) => {
          ...
          settled = true;
          clearTimeout(timeout);
          reject(new Error(`Rok Desktop daemon exited before reporting ready${signal ? ` (${signal})` : ` (code ${code ?? 'unknown'})`}`));
        });
      });
    } catch (error) {
    ...
    if (!isProcessRunning(child.pid)) {
      serveSpin?.error('Failed to start Rok Desktop');
    ...
    const serveResult = {
      ...
      logs: `rok-desktop logs -p ${resolvedPort}`,
      launchMode: 'daemon',
    };
    ...
    if (!options.suppressStartupSummary && showOutput) {
      clackIntro('Rok Desktop Started');
