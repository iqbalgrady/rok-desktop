...
async function logsCommand(options) {
  ...
  if (options.all) {
    targets = running;
    if (targets.length === 0) {
      throw new Error('No running Rok Desktop instance found.');
    }
  ...
  if (isJsonMode(options)) {
    if (options.follow) {
      throw new Error('`rok-desktop logs --json` requires `--no-follow` for deterministic JSON output.');
    }
