...
async function getPidFilePath(port) {
  return path.join(getRunDir(), `rok-desktop-${port}.pid`);
}

async function getInstanceFilePath(port) {
  return path.join(getRunDir(), `rok-desktop-${port}.json`);
}
...
function isOpenchamberCmdline(cmdline) {
  ...
  // Every install path contains the "rok-desktop" segment — the npm package
  // (@rok-desktop/server) and the source checkout both do, for the foreground
  ...
  return cmdline.toLowerCase().includes('rok-desktop');
}

// Liveness + identity — "is the Rok Desktop instance recorded in a pid file
