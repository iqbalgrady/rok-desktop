...
import {
  ...
  commands,
  discoverRokDesktopInstanceOnPort,
...
async function withTempRokDesktopDataDir(fn) {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'rok-desktop-cli-test-'));
...
async function startMockRokDesktopServer(options = {}) {
...
function spawnRokDesktopLikeIdleProcess() {
  return spawn(process.execPath, ['-e', 'setInterval(() => {}, 1000)', 'rok-desktop-idle'], { stdio: 'ignore' });
}

function spawnRokDesktopLikeHungServer(port) {
  const script = `
    ...
    setInterval(() => {}, 1000);
  `;
  return spawn(process.execPath, ['-e', script, 'rok-desktop-hung-server'], { stdio: 'ignore' });
}
...
describe('cli args', () => {
  ...
  it('parses explicit connect-url server overrides', () => {
    const parsed = parseArgs(['connect-url', '--server', 'https://rok-desktop.example.com', '--port', '3002']);

    expect(parsed.command).toBe('connect-url');
    expect(parsed.options.server).toBe('https://rok-desktop.example.com');
...
describe('serve host resolution', () => {
  it('uses ROK_DESKTOP_HOST when --host is not provided', () => {
    const previous = process.env.ROK_DESKTOP_HOST;
    process.env.ROK_DESKTOP_HOST = '192.0.2.20';
    try {
      expect(resolveServeHost(undefined)).toBe('192.0.2.20');
    } finally {
      if (typeof previous === 'string') {
        process.env.ROK_DESKTOP_HOST = previous;
      } else {
        delete process.env.ROK_DESKTOP_HOST;
      }
    }
  });

  it('prefers explicit --host over ROK_DESKTOP_HOST', () => {
    const previous = process.env.ROK_DESKTOP_HOST;
    process.env.ROK_DESKTOP_HOST = '192.0.2.20';
    try {
...
describe('CLI HTTP helpers', () => {
  it('retries UI-authenticated API requests with the stored instance password', async () => {
    await withTempRokDesktopDataDir(async () => {
      ...
      try {
        const { response, body } = await requestJson(port, '/api/rok-desktop/tunnel/start', {
          method: 'POST',
...
describe('cli entry detection', () => {
  const modulePath = '/tmp/rok-desktop/bin/cli.js';
  const moduleUrl = pathToFileURL(modulePath).href;

  it('resolves symlinked entry paths before comparing', () => {
    const symlinkPath = '/usr/local/bin/rok-desktop';
  ...
  it('accepts wrapper binary name fallback when requested', () => {
    const wrapperPath = '/home/user/.local/bin/rok-desktop';
    expect(isModuleCliExecution(wrapperPath, moduleUrl, undefined, 'rok-desktop')).toBe(true);
  });
...
describe('isOpenchamberCmdline', () => {
  it('accepts Rok Desktop CLI and daemon cmdlines', () => {
    expect(isOpenchamberCmdline('node /x/@rok-desktop/server/bin/cli.js serve')).toBe(true);
    expect(isOpenchamberCmdline('node /x/@rok-desktop/server/server/index.js --port 9090')).toBe(true);
    expect(isOpenchamberCmdline('bun /home/u/projects/rok-desktop/packages/web/server/index.js --port 3001')).toBe(true);
  });
...
describe('isOpenchamberProcessRunning', () => {
  ...
  it.skipIf(process.platform !== 'linux' && process.platform !== 'darwin')(
    'returns false for a live non-Rok Desktop PID',
    async () => {
...
describe('lifecycle instance discovery', () => {
  it('does not attribute a desktop runtime response to a different explicit port', async () => {
    await withTempRokDesktopDataDir(async (dir) => {
      fs.writeFileSync(path.join(dir, 'settings.json'), JSON.stringify({ desktopLocalPort: 57123 }, null, 2));

      const instance = await discoverRokDesktopInstanceOnPort(3003, {
  ...
  it('attributes a desktop runtime response to its configured desktop port', async () => {
    await withTempRokDesktopDataDir(async (dir) => {
      ...
      const instance = await discoverRokDesktopInstanceOnPort(57123, {
        fetchImpl: async () => createMockJsonResponse({ runtime: 'desktop', pid: 934 }),
      });
  ...
  it('uses the live system-info pid instead of a stale Rok Desktop-looking pid-file pid', async () => {
    await withTempRokDesktopDataDir(async () => {
      const port = 45127;
  ...
  it('discovers an explicit live Rok Desktop port without a pid-file registry entry', async () => {
    await withTempRokDesktopDataDir(async () => {
      const port = 45125;
      const instances = await discoverLifecycleInstances(
  ...
  it('cleans a matched pid-file entry without stopping it when the recorded port is free', async () => {
    await withTempRokDesktopDataDir(async () => {
      ...
      const child = spawnRokDesktopLikeIdleProcess();
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
        await new Promise((resolve) => setTimeout(resolve, 150));
...
describe('lifecycle commands with unmanaged explicit ports', () => {
  it('serve refuses to start on a live Rok Desktop port without requiring pid files', async () => {
    await withTempRokDesktopDataDir(async () => {
      const server = await startMockRokDesktopServer();
      try {
  ...
  it('stop --port can recover a matched pid-file instance whose HTTP endpoint is unresponsive', async () => {
    await withTempRokDesktopDataDir(async () => {
      const port = await allocateLoopbackPort();
      const child = spawnRokDesktopLikeHungServer(port);
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
  ...
  it('plain stop ignores a stale CLI registry entry that resolves to desktop runtime', async () => {
    await withTempRokDesktopDataDir(async () => {
      const server = await startMockRokDesktopServer({ runtime: 'desktop' });
      const child = spawn('sleep', ['30'], { stdio: 'ignore' });
