...
import { registerRokDesktopRoutes } from './lib/opencode/rok-desktop-routes.js';
...
const DESKTOP_NOTIFY_PREFIX = '[RokDesktopNotify] ';
const uiNotificationClients = new Set();
const uiNotificationWsClients = new Set();
const uiRokDesktopEventClients = new Set();
...
const SSE_PATH_PREFIXES = [
  ...
  '/api/rok-desktop/events',
  '/api/rok-desktop/realtime-proxy/sse',
];

function shouldSkipCompression(req, res) {
  if (process.env.ROK_DESKTOP_RUNTIME === 'desktop') {
    return true;
  }
...
const shouldSkipApiCompression = () => {
  if (isEnvFlagEnabled(process.env.ROK_DESKTOP_SKIP_API_COMPRESSION)) return true;
  if (isEnvFlagEnabled(process.env.ROK_DESKTOP_COMPRESS_API)) return false;
  if (isEnvFlagDisabled(process.env.ROK_DESKTOP_COMPRESS_API)) return true;
  return process.env.ROK_DESKTOP_RUNTIME === 'desktop';
};

const ROK_DESKTOP_VERBOSE_REQUEST_LOGS = isEnvFlagEnabled(process.env.ROK_DESKTOP_VERBOSE_REQUEST_LOGS);
...
const ROK_DESKTOP_USER_CONFIG_ROOT = path.join(os.homedir(), '.config', 'rok-desktop');
const ROK_DESKTOP_USER_THEMES_DIR = path.join(ROK_DESKTOP_USER_CONFIG_ROOT, 'themes');
const ROK_DESKTOP_PROJECTS_CONFIG_DIR = path.join(ROK_DESKTOP_USER_CONFIG_ROOT, 'projects');
...
const themeRuntime = createThemeRuntime({
  fsPromises,
  path,
  themesDir: ROK_DESKTOP_USER_THEMES_DIR,
...
const ROK_DESKTOP_DATA_DIR = process.env.ROK_DESKTOP_DATA_DIR
  ? path.resolve(process.env.ROK_DESKTOP_DATA_DIR)
  : path.join(os.homedir(), '.config', 'rok-desktop');
const SETTINGS_FILE_PATH = path.join(ROK_DESKTOP_DATA_DIR, 'settings.json');
const PUSH_SUBSCRIPTIONS_FILE_PATH = path.join(ROK_DESKTOP_DATA_DIR, 'push-subscriptions.json');
const APNS_TOKENS_FILE_PATH = path.join(ROK_DESKTOP_DATA_DIR, 'apns-tokens.json');
const REMOTE_CLIENTS_FILE_PATH = path.join(ROK_DESKTOP_DATA_DIR, 'remote-clients.json');
const CLOUDFLARE_MANAGED_REMOTE_TUNNELS_FILE_PATH = path.join(ROK_DESKTOP_DATA_DIR, 'cloudflare-managed-remote-tunnels.json');
const CLOUDFLARE_LEGACY_NAMED_TUNNELS_FILE_PATH = path.join(ROK_DESKTOP_DATA_DIR, 'cloudflare-named-tunnels.json');
...
const projectConfigRuntime = createProjectConfigRuntime({
  fsPromises,
  path,
  projectsDirPath: ROK_DESKTOP_PROJECTS_CONFIG_DIR,
});
...
const ENV_DESKTOP_NOTIFY = (() => {
  ...
  return /rok-desktop-server/i.test(argv0) || /rok-desktop-server/i.test(argv1);
})();
...
const processForwardedEventPayload = (payload, emitSyntheticEvent) => {
  ...
  emitSyntheticEvent({
    type: 'rok-desktop:session-status',
    properties: {
      sessionID: sessionId,
      status,
      timestamp: Date.now(),
  ...
  emitSyntheticEvent({
    type: 'rok-desktop:session-activity',
    properties: {
      sessionId,
...
const scheduledTasksRuntime = createScheduledTasksRuntime({
  ...
  emitTaskRunEvent: (event) => {
    for (const client of uiRokDesktopEventClients) {
      try {
        writeSseEvent(client, {
          type: 'rok-desktop:scheduled-task-ran',
          properties: {
        ...
        uiRokDesktopEventClients.delete(client);
      }
    }
  },
  logger: console,
...
async function main(options = {}) {
  ...
  const effectiveBindHost = host
    || (typeof process.env.ROK_DESKTOP_HOST === 'string' && process.env.ROK_DESKTOP_HOST.trim().length > 0
      ? process.env.ROK_DESKTOP_HOST.trim()
      : '127.0.0.1');
  const uiPassword = typeof options.uiPassword === 'string'
    ? options.uiPassword
    : (typeof process.env.ROK_DESKTOP_UI_PASSWORD === 'string' ? process.env.ROK_DESKTOP_UI_PASSWORD : null);
  if (
  ...
  const apiOnly = options.apiOnly === true || isEnvFlagEnabled(process.env.ROK_DESKTOP_API_ONLY);
  ...
  console.log(`Starting Rok Desktop on port ${port === 0 ? 'auto' : port}`);
  ...
  const bootstrapResult = bootstrapRuntime.setupBaseRoutes(app, {
    ...
    verboseRequestLogs: ROK_DESKTOP_VERBOSE_REQUEST_LOGS,
    uiPassword,
    tunnelAuthController,
    ...
    server,
    __dirname,
    rokDesktopDataDir: ROK_DESKTOP_DATA_DIR,
  ...
  await featureRoutesRuntime.registerRoutes(app, {
    ...
    rokDesktopDataDir: ROK_DESKTOP_DATA_DIR,
    rokDesktopUserConfigRoot: ROK_DESKTOP_USER_CONFIG_ROOT,
    normalizeDirectoryPath,
    resolveProjectDirectory,
    ...
    projectConfigRuntime,
    scheduledTasksRuntime,
    getRokDesktopEventClients: () => uiRokDesktopEventClients,
    writeSseEvent,
  });
