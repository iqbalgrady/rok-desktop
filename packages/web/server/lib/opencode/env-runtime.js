...
export const createOpenCodeEnvRuntime = (deps) => {
  ...
  const resolveNodeCliPath = () => {
    const explicit = [process.env.NODE_BINARY, process.env.ROK_DESKTOP_NODE_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const resolveBunCliPath = () => {
    const explicit = [process.env.BUN_BINARY, process.env.ROK_DESKTOP_BUN_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const createConfiguredWslOpencodeError = (raw) => new Error(
    `Configured settings.opencodeBinary uses WSL but Rok Desktop could not resolve a WSL OpenCode command: ${raw}. Ensure WSL is available and opencode is installed in the configured distro.`
  );
  ...
  const applyOpencodeBinaryFromSettings = async (options = {}) => {
    ...
    try {
      ...
      if (explicitWslPath && explicitWslPath[1] && explicitWslPath[1].trim().length > 0) {
        ...
        console.warn(`Configured settings.opencodeBinary uses WSL, which is no longer supported by Rok Desktop desktop: ${raw}`);
        return null;
      }
      ...
      if (process.platform === 'win32' && (isWslExecutableValue(raw) || isWslExecutableValue(normalized || ''))) {
        ...
        console.warn(`Configured settings.opencodeBinary points to WSL, which is no longer supported by Rok Desktop desktop: ${raw}`);
        return null;
      }
  ...
  const resolveGitBinaryForSpawn = () => {
    ...
    const explicit = [process.env.GIT_BINARY, process.env.ROK_DESKTOP_GIT_BINARY]
