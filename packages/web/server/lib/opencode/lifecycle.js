...
const HEALTH_CHECK_TIMEOUT_MS = parsePositiveInt(process.env.ROK_DESKTOP_OPENCODE_HEALTH_TIMEOUT_MS, 5000);
const HEALTH_CHECK_MAX_CONSECUTIVE_FAILURES = parsePositiveInt(
  process.env.ROK_DESKTOP_OPENCODE_HEALTH_CONSECUTIVE_FAILURES,
  20
);
const HEALTH_CHECK_INTERVAL_OVERRIDE_MS = parsePositiveInt(process.env.ROK_DESKTOP_OPENCODE_HEALTH_INTERVAL_MS, 0);
const HEALTH_CHECK_RESULT_CACHE_MS = parsePositiveInt(process.env.ROK_DESKTOP_OPENCODE_HEALTH_CACHE_MS, 750);
...
export const createOpenCodeLifecycleRuntime = (deps) => {
  ...
  const createManagedOpenCodeServerProcess = async ({ hostname, port, timeout, cwd, env: processEnv, shellEnvKeysCount = 0 }) => {
    let binary = (process.env.ROKCODE_BINARY || 'rokcode').trim() || 'rokcode';
    ...
    const url = await new Promise((resolve, reject) => {
      ...
      const onStdout = (chunk) => {
        ...
        for (const line of lines) {
          if (!line.startsWith('rokcode server listening')) continue;
    ...
    // actual host (Electron sets ROK_DESKTOP_RUNTIME='desktop'; the standalone
    ...
    registerManagedProcess({
      ...
      port,
      binary,
      runtime: process.env.ROK_DESKTOP_RUNTIME || 'web',
    });
