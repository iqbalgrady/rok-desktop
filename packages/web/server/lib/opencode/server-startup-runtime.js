        ...
        try {
          ...
          if (typeof process.send === 'function') {
            if (!process.connected) {
              throw new Error('Rok Desktop startup IPC channel disconnected before ready notification');
            }

            await new Promise((resolveReadyNotification, rejectReadyNotification) => {
              try {
                process.send({ type: 'rok-desktop:ready', port: activePort }, (error) => {
                  if (error) {
          ...
          console.log(`Rok Desktop server listening on ${bindHost}:${activePort}`);
