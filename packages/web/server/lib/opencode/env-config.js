export const resolveOpenCodeEnvConfig = (options = {}) => {
  ...
  const configuredOpenCodeHost = (() => {
    const raw = typeof env.OPENCODE_HOST === 'string' ? env.OPENCODE_HOST.trim()
      : typeof env.ROKCODE_HOST === 'string' ? env.ROKCODE_HOST.trim()
      : '';
    if (!raw) return null;

    const warnInvalidHost = (reason) => {
      logger.warn(`[config] Ignoring OPENCODE_HOST/ROKCODE_HOST=${JSON.stringify(raw)}: ${reason}`);
    };
  ...
  // OPENCODE_HOST/ROKCODE_HOST takes precedence over port env vars when set
  const effectivePort = configuredOpenCodeHost?.port ?? configuredOpenCodePort;

  const configuredOpenCodeHostname = (() => {
    const raw = env.ROK_DESKTOP_OPENCODE_HOSTNAME;
    ...
    if (!trimmed) {
      logger.warn(
        `[config] Ignoring ROK_DESKTOP_OPENCODE_HOSTNAME=${JSON.stringify(raw)}: empty after trimming`,
      );
      return '127.0.0.1';
    }
    return trimmed;
