export const registerProjectIconRoutes = (app, dependencies) => {
  ...
  const projectIconsDirPath = path.join(rokDesktopDataDir, 'project-icons');
  ...
  const applyProjectIconSvgTheme = (svgMarkup, themeVariant, iconColor) => {
    ...
    const overrideStyle = `<style data-rok-desktop-theme-icon="1">:root{color:${color}!important;}</style>`;
