export const registerRokDesktopRoutes = (app, dependencies) => {
  const {
    fs,
    path,
    process,
  ...
  app.get('/api/rok-desktop/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/rok-desktop/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(rokDesktopDataDir, 'run', `rok-desktop-${currentPort}.json`);
      ...
      let restartCmdFallback = `rok-desktop serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(rokDesktopDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== Rok Desktop update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting Rok Desktop...
              ${restartCmd || 'echo Service manager will restart Rok Desktop.'}
            ) else (
              echo Update failed
              exit /b 1
            )
  ...
  app.get('/api/rok-desktop/models-metadata', async (_req, res) => {
    const now = Date.now();
