...
const PACKAGE_NAME = '@rok-desktop/server';
...
const UPDATE_CHECK_URL = process.env.ROK_DESKTOP_UPDATE_API_URL || 'https://api.rok-desktop.dev/v1/update/check';

function getRokDesktopConfigDir() {
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA;
    if (appData) return path.join(appData, 'rok-desktop');
  }

  return path.join(os.homedir(), '.config', 'rok-desktop');
}
...
function getOrCreateInstallId(scope = 'web') {
  const configDir = getRokDesktopConfigDir();
...
function getOwnedPackagePathsFromGlobalBins(pm) {
  ...
  for (const binDir of getGlobalBinDirs(pm)) {
    const binaryName = process.platform === 'win32' ? 'rok-desktop.cmd' : 'rok-desktop';
...
function isPackageInstalledWith(pm) {
  try {
    ...
    return result.stdout.includes(PACKAGE_NAME) || result.stdout.includes('rok-desktop');
  } catch {
    return false;
  }
}
  ...
  if (currentVersion !== 'unknown') {
    ...
    if (remote) {
      ...
      return {
        ...remote,
        packageManager: pm,
        updateCommand: 'rok-desktop update',
      };
    }
  }
