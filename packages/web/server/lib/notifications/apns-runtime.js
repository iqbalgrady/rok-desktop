...
const DEFAULT_BUNDLE_ID = 'com.ecc.rokcode';
const DEFAULT_RELAY_URL = 'https://api.rok-desktop.dev/v1/push/send';
...
export const createApnsRuntime = (deps) => {
  ...
  const resolveApnsConfig = async () => {
    let keyId = trimmedEnv('ROK_DESKTOP_APNS_KEY_ID');
    let teamId = trimmedEnv('ROK_DESKTOP_APNS_TEAM_ID');
    let bundleId = trimmedEnv('ROK_DESKTOP_APNS_BUNDLE_ID');
    let environment = (trimmedEnv('ROK_DESKTOP_APNS_ENVIRONMENT') || '').toLowerCase();
    let p8 = normalizePem(process.env.ROK_DESKTOP_APNS_P8 || '');

    const p8Path = trimmedEnv('ROK_DESKTOP_APNS_P8_PATH');
    if (!p8 && p8Path) {
      try {
        p8 = (await fsPromises.readFile(p8Path, 'utf8')).trim();
      } catch (error) {
        console.warn('[APNs] Failed to read ROK_DESKTOP_APNS_P8_PATH:', error?.message ?? error);
      }
    }
  ...
  const resolveRelayConfig = () => {
    ...
    return {
      ...
      environment:
        (trimmedEnv('ROK_DESKTOP_APNS_ENVIRONMENT') || 'sandbox').toLowerCase() === 'production'
          ? 'production'
          : 'sandbox',
    };
  };
  ...
  const sendViaRelay = async (deviceTokens, payload, relay) => {
    ...
    const title = typeof payload?.title === 'string' && payload.title.length > 0 ? payload.title : 'Rok Desktop';
    const { privateKey, publicJwk } = await getOrCreateRelayKeypair();
    const ts = Date.now();
  ...
  const sendViaDirectApns = async (deviceTokens, payload) => {
    ...
    if (!config) {
      if (!warnedUnconfigured) {
        ...
        console.warn(
          '[APNs] Relay disabled and no direct config; set ROK_DESKTOP_APNS_KEY_ID / ROK_DESKTOP_APNS_TEAM_ID / ROK_DESKTOP_APNS_P8 for direct send.',
        );
      }
      return;
    }
