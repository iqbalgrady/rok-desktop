...
const PREVIEW_BRIDGE_SCRIPT_ID = 'rok-desktop-preview-bridge';
...
const PREVIEW_BRIDGE_SCRIPT = String.raw`(() => {
  if (window.__rokDesktopPreviewBridgeInstalled) return;
  window.__rokDesktopPreviewBridgeInstalled = true;

  const SOURCE = 'rok-desktop-preview-bridge';
  const VERSION = 1;
  const MAX_TEXT = 500;
  const MAX_ARG = 1000;
  const TARGET_ORIGIN = typeof window.__rokDesktopPreviewTargetOrigin === 'string' ? window.__rokDesktopPreviewTargetOrigin : '';
  ...
  const installColorSchemeMatchMediaPatch = () => {
    if (window.__rokDesktopPreviewColorSchemePatched || typeof window.matchMedia !== 'function') return;
    window.__rokDesktopPreviewColorSchemePatched = true;
  ...
  const applyPreviewColorScheme = (scheme) => {
    ...
    try {
      ...
      root.dataset.rok-desktopPreviewColorScheme = next;
  ...
  const installViteHmrProxyPatch = () => {
    if (window.__rokDesktopViteHmrProxyPatched || typeof window.WebSocket !== 'function') return;
    window.__rokDesktopViteHmrProxyPatched = true;
    ...
    function RokDesktopPreviewWebSocket(url, protocols) {
      const protocolList = Array.isArray(protocols) ? protocols : [protocols];
    ...
    RokDesktopPreviewWebSocket.prototype = NativeWebSocket.prototype;
    Object.setPrototypeOf(RokDesktopPreviewWebSocket, NativeWebSocket);
    Object.defineProperty(RokDesktopPreviewWebSocket, 'name', { value: 'WebSocket' });
    window.WebSocket = RokDesktopPreviewWebSocket;
  };

  const installAppRequestProxyPatch = () => {
    if (window.__rokDesktopAppRequestProxyPatched) return;
    window.__rokDesktopAppRequestProxyPatched = true;
    const proxyMatch = window.location.pathname.match(/^(\/api\/preview\/proxy\/[a-f0-9]{16,64})(?:\/|$)/i);
    if (!proxyMatch) return;
    const proxyBase = proxyMatch[1];
    const currentSearchParams = new URL(window.location.href).searchParams;
    ...
    if (typeof window.EventSource === 'function') {
      ...
      function RokDesktopPreviewEventSource(url, eventSourceInitDict) {
        return new NativeEventSource(proxiedUrl(String(url)), eventSourceInitDict);
      }
      RokDesktopPreviewEventSource.prototype = NativeEventSource.prototype;
      Object.setPrototypeOf(RokDesktopPreviewEventSource, NativeEventSource);
      Object.defineProperty(RokDesktopPreviewEventSource, 'name', { value: 'EventSource' });
      window.EventSource = RokDesktopPreviewEventSource;
    }

    if (typeof window.WebSocket === 'function') {
      const NativeWebSocket = window.WebSocket;
      function RokDesktopPreviewAppWebSocket(url, protocols) {
        const nextUrl = proxiedWebSocketUrl(String(url));
        return arguments.length === 1
          ? new NativeWebSocket(nextUrl)
          : new NativeWebSocket(nextUrl, protocols);
      }
      RokDesktopPreviewAppWebSocket.prototype = NativeWebSocket.prototype;
      Object.setPrototypeOf(RokDesktopPreviewAppWebSocket, NativeWebSocket);
      Object.defineProperty(RokDesktopPreviewAppWebSocket, 'name', { value: 'WebSocket' });
      window.WebSocket = RokDesktopPreviewAppWebSocket;
    }
  };
  ...
  const sendHover = (event) => {
    ...
    pendingHover = event;
    if (window.__rokDesktopPreviewHoverFrame) return;
    window.__rokDesktopPreviewHoverFrame = window.requestAnimationFrame(() => {
      window.__rokDesktopPreviewHoverFrame = 0;
  ...
  window.addEventListener('message', (event) => {
    ...
    if (!data || data.source !== 'rok-desktop-preview-parent' || data.version !== VERSION) return;
...
export const normalizeProxyTargetUrl = (rawUrl, { allowExternal = false } = {}) => {
  ...
  // Only keep origin here; the proxy path is preserved on the Rok Desktop side.
...
const appendProxyAuthToProxyUrl = (value, { previewToken = '', urlAuthToken = '' } = {}) => {
  ...
  try {
    const parsed = new URL(value, 'http://rok-desktop-preview.local');
...
export const createPreviewProxyRuntime = ({
  ...
  const attach = (app, {
    ...
    const injectPreviewBridge = (bodyText, targetOrigin, bridgeNonce) => {
      ...
      const targetOriginScript = `<script${nonceAttr}>window.__rokDesktopPreviewTargetOrigin=${JSON.stringify(targetOrigin || '')};</script>`;
    ...
    const proxy = createProxyMiddleware({
      ...
      on: {
        proxyReq: (proxyReq, req) => {
          ...
          // Keep local dev servers from receiving Rok Desktop credentials.
          proxyReq.removeHeader('cookie');
          proxyReq.removeHeader('authorization');
          proxyReq.removeHeader('x-rok-desktop-ui-session');
        ...
        proxyRes: responseInterceptor(async (responseBuffer, proxyRes, req, res) => {
          ...
          // Allow the dev server response to be framed inside Rok Desktop even
