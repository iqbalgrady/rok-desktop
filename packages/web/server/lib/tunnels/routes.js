export const createTunnelRoutesRuntime = (dependencies) => {
  ...
  const registerRoutes = (app) => {
    app.get('/api/rok-desktop/tunnel/check', async (req, res) => {
      try {
    ...
    app.post('/api/rok-desktop/tunnel/doctor', handleTunnelDoctor);
    app.get('/api/rok-desktop/tunnel/doctor', handleTunnelDoctor);

    app.get('/api/rok-desktop/tunnel/providers', (_req, res) => {
      const providers = tunnelProviderRegistry.listCapabilities();
      return res.json({ providers });
    });

    app.get('/api/rok-desktop/tunnel/status', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.put('/api/rok-desktop/tunnel/managed-remote-token', async (req, res) => {
      try {
    ...
    app.post('/api/rok-desktop/tunnel/start', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.post('/api/rok-desktop/tunnel/stop', (_req, res) => {
      let revokedBootstrapCount = 0;
      let invalidatedSessionCount = 0;
      const activeTunnelId = tunnelAuthController.getActiveTunnelId();
