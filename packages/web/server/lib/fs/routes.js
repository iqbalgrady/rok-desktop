...
const createGitReadCacheTtlMs = () => {
  const raw = Number(process.env.ROK_DESKTOP_GIT_READ_CACHE_TTL_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 30 * 1000;
};

const createGitCheckIgnoreTimeoutMs = () => {
  const raw = Number(process.env.ROK_DESKTOP_GIT_CHECK_IGNORE_TIMEOUT_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 2500;
};
...
const resolveWorkspacePath = ({ targetPath, baseDirectory, path, os, normalizeDirectoryPath, rokDesktopUserConfigRoot }) => {
  ...
  if (isPathWithinRoot(resolved, rokDesktopUserConfigRoot, path, os)) {
    return { ok: true, base: path.resolve(rokDesktopUserConfigRoot), resolved };
  }
...
const resolveWorkspacePathFromContext = async ({ req, targetPath, resolveProjectDirectory, path, os, normalizeDirectoryPath, rokDesktopUserConfigRoot }) => {
...
const resolveReadPathFromContext = async ({ req, targetPath, scope, resolveProjectDirectory, path, os, fsPromises, normalizeDirectoryPath, rokDesktopUserConfigRoot }) => {
