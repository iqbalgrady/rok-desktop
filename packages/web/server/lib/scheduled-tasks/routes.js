...
export const registerScheduledTaskRoutes = (app, dependencies) => {
  ...
  app.get('/api/rok-desktop/scheduled-tasks/status', async (_req, res) => {
    try {
  ...
  app.get('/api/rok-desktop/events', (req, res) => {
    ...
    const clients = getRokDesktopEventClients();
    clients.add(res);

    try {
      writeSseEvent(res, {
        type: 'rok-desktop:event-stream-ready',
        properties: {
