...
const isUrlAuthReadableHttpPath = (pathname) => {
  return pathname === '/api/event'
    ...
    || pathname === '/api/rok-desktop/events'
    || pathname === '/api/rok-desktop/realtime-proxy/sse'
...
const isUrlAuthWebSocketPath = (pathname) => {
  return pathname === '/api/event/ws'
    ...
    || pathname === '/api/rok-desktop/realtime-proxy/ws'
...
const JWT_SECRET_FILE = path.join(ROK_DESKTOP_DATA_DIR, 'jwt-secret');
...
function getOrCreateJwtSecret() {
  ...
  try {
    fs.mkdirSync(ROK_DESKTOP_DATA_DIR, { recursive: true });
