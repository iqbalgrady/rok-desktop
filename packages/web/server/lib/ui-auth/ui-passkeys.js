...
const DEFAULT_RP_NAME = 'Rok Desktop';
...
const PASSKEY_STORE_FILE = path.join(ROK_DESKTOP_DATA_DIR, 'ui-passkeys.json');
...
export const createUiPasskeys = ({
  ...
  const beginRegistration = async (req, { label } = {}) => {
    ...
    const options = await generateRegistrationOptions({
      ...
      rpID,
      userID,
      userName: 'rok-desktop-ui',
      userDisplayName: 'Rok Desktop UI',
  ...
  const finishAuthentication = async (payload) => {
    ...
    if (!passkey) {
      const error = new Error('That passkey is not registered for this Rok Desktop instance');
      error.statusCode = 404;
      throw error;
    }
