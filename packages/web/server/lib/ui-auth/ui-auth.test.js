...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rok-desktop-ui-auth-test-'));
process.env.ROK_DESKTOP_DATA_DIR = dataDir;
...
describe('ui auth client credential seam', () => {
  ...
  it('issues desktop client tokens with the UI session expiry', async () => {
    ...
    expect(createClientInput.label).toBe('Rok Desktop Desktop');
