...
Full project overview, screenshots, and all features: [github.com/rokcode/rok-desktop](https://github.com/rokcode/rok-desktop)

## Install

```bash
curl -fsSL https://raw.githubusercontent.com/rokcode/rok-desktop/main/scripts/install.sh | bash
```

Or install manually: `bun add -g @rok-desktop/server` (or npm, pnpm, yarn).
...
`startup enable` snapshots your current environment into the native service so startup behaves like you launched `rok-desktop` from the same shell. This preserves provider tokens, PATH, SSH agent settings, and other CLI auth/config env vars. Use `--no-env-snapshot` for a minimal service env.
...
This creates a remote client token and prints an `rok-desktop://connect?...` link. The link contains the server URL, token, label, and payload version. In OpenChamber Desktop, paste it in **Settings -> Remote Instances -> Direct Instances -> Import Link** to add that server as an Instance.
