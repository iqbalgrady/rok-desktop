...
//
// Surfaces a glanceable, always-visible view of Rok Desktop's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'Rok Desktop — no active sessions';
  const bits = [];
  ...
  return `Rok Desktop — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    template.push(
      ...
      { label: 'Show Rok Desktop', click: () => onAction({ type: 'show-main-window' }) },
      { type: 'separator' },
      { label: 'Quit Rok Desktop', click: () => onAction({ type: 'quit' }) },
    );
