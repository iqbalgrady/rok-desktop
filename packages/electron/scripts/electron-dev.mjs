...
const preferredHmrUiPort = Number(process.env.ROK_DESKTOP_HMR_UI_PORT || '5173');
const preferredHmrApiPort = Number(process.env.ROK_DESKTOP_HMR_API_PORT || '3901');
...
function spawnProcess(command, args, options = {}) {
  ...
  return spawn(spawnCommand, spawnArgs, {
    cwd: repoRoot,
    env: { ...process.env, ROK_DESKTOP_ELECTRON_DEV: '1' },
    stdio: 'inherit',
...
async function main() {
  const useBundledUi = process.env.ROK_DESKTOP_ELECTRON_USE_BUNDLED_UI === '1';
  let devServer = null;
  let hmrApiPort = '';
  let hmrUiPort = '';
  ...
  if (useBundledUi) {
    ...
    devServer = spawnProcess('node', ['./scripts/dev-web-hmr.mjs'], {
      env: {
        ...process.env,
        ROK_DESKTOP_ELECTRON_DEV: '1',
        ROK_DESKTOP_HMR_UI_PORT: hmrUiPort,
        ROK_DESKTOP_HMR_API_PORT: hmrApiPort,
  ...
  const electron = spawnProcess('npx', ['electron', './main.mjs'], {
    ...
    env: {
      ...process.env,
      ROK_DESKTOP_ELECTRON_DEV: '1',
      ...(useBundledUi ? { ROK_DESKTOP_ELECTRON_USE_BUNDLED_UI: '1' } : {}),
      ROK_DESKTOP_HMR_UI_PORT: hmrUiPort,
      ROK_DESKTOP_HMR_API_PORT: hmrApiPort,
      ROK_DESKTOP_DISABLE_PWA_DEV: '1',
    },
