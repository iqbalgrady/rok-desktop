...
const SSH_STATUS_EVENT = 'rok-desktop:ssh-instance-status';
...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded Rok Desktop health');
};
...
export class ElectronSshManager {
  ...
  sanitizeInstance(instance) {
    ...
    return {
      ...
      auth: {
        ...
        ...(this.sanitizeStoredSecret(instance?.auth?.rokDesktopPassword) ? { rokDesktopPassword: this.sanitizeStoredSecret(instance.auth.rokDesktopPassword) } : {}),
      },
      portForwards,
    };
  }
  ...
  async issueClientToken(localUrl, rokDesktopPassword) {
    const password = typeof rokDesktopPassword === 'string' ? rokDesktopPassword.trim() : '';
    if (!password) return '';
    ...
    const loginResponse = await fetch(new URL('/auth/session', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({
        ...
        clientLabel: 'Rok Desktop Desktop SSH',
      }),
    });
    if (!loginResponse.ok) {
      throw new Error(`Configured Rok Desktop UI password was rejected by forwarded server (status ${loginResponse.status})`);
    }
    ...
    const tokenResponse = await fetch(new URL('/api/client-auth/clients', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({ label: 'Rok Desktop Desktop SSH' }),
    });
  ...
  configuredRokDesktopPassword(instance) {
    const secret = instance?.auth?.rokDesktopPassword;
  ...
  async currentRemoteRokDesktopVersion(parsed, controlPath) {
    try {
      const output = await runRemoteCommand(parsed, controlPath, 'rok-desktop --version 2>/dev/null || true');
  ...
  async installRokDesktopManaged(parsed, controlPath, version, preferred) {
    ...
    if (preferred === 'bun') {
      if (hasBun) commands.push(`bun add -g @rok-desktop/server@${version}`);
      if (hasNpm) commands.push(`npm install -g @rok-desktop/server@${version}`);
    ...
    throw lastError || new Error('Failed to install Rok Desktop on remote host');
  }

  async probeRemoteSystemInfo(parsed, controlPath, port, rokDesktopPassword) {
    const authPayload = rokDesktopPassword ? JSON.stringify({ password: rokDesktopPassword }) : '{}';
    const authEnabled = rokDesktopPassword ? '1' : '0';
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (rokDesktopPassword && authStatus !== 200) {
          throw new Error(`Remote Rok Desktop requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote Rok Desktop requires UI authentication on /api/system/info; configure Rok Desktop UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote Rok Desktop probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async remoteServerRunning(parsed, controlPath, port, rokDesktopPassword) {
    try {
      await this.probeRemoteSystemInfo(parsed, controlPath, port, rokDesktopPassword);
      return true;
    } catch {
      return false;
    }
  ...
  async startRemoteServerManaged(parsed, controlPath, instance, desiredPort) {
    let envPrefix = 'ROK_DESKTOP_RUNTIME=ssh-remote';
    const secret = this.configuredRokDesktopPassword(instance);
    if (secret) {
      envPrefix += ` ROK_DESKTOP_UI_PASSWORD=${shellQuote(secret)}`;
    }
    const output = await runRemoteCommand(parsed, controlPath, `${envPrefix} rok-desktop serve --hostname 127.0.0.1 --port ${desiredPort}`);
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteOpenchamber.mode === 'external') {
      if (!instance.remoteOpenchamber.preferredPort) {
        throw new Error('External mode requires a preferred remote Rok Desktop port');
      }
      const port = instance.remoteOpenchamber.preferredPort;
      this.setStatus(instance.id, 'server_detecting', 'Probing external Rok Desktop server', null, null, port, false, 0, false);
      await this.probeRemoteSystemInfo(parsed, controlPath, port, this.configuredRokDesktopPassword(instance));
      return { remotePort: port, startedByUs: false };
    }

    this.setStatus(instance.id, 'remote_probe', 'Checking remote Rok Desktop installation');
    const installedVersion = await this.currentRemoteRokDesktopVersion(parsed, controlPath);
    if (!installedVersion) {
      this.setStatus(instance.id, 'installing', 'Installing Rok Desktop on remote host');
      await this.installRokDesktopManaged(parsed, controlPath, this.appVersion, instance.remoteOpenchamber.installMethod);
    } else if (installedVersion !== this.appVersion) {
      this.setStatus(instance.id, 'updating', `Updating remote Rok Desktop from ${installedVersion} to ${this.appVersion}`);
      await this.installRokDesktopManaged(parsed, controlPath, this.appVersion, instance.remoteOpenchamber.installMethod);
    }

    this.setStatus(instance.id, 'server_detecting', 'Detecting managed Rok Desktop server');
    let remotePort = instance.remoteOpenchamber.preferredPort || null;
    let startedByUs = false;
    if (remotePort && !(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredRokDesktopPassword(instance)))) {
      remotePort = null;
    }
    if (!remotePort) {
      this.setStatus(instance.id, 'server_starting', 'Starting managed Rok Desktop server');
    ...
    if (!(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredRokDesktopPassword(instance)))) {
      throw new Error('Managed Rok Desktop server failed to become reachable');
    }
  ...
  async connectBlocking(instance) {
    ...
    const clientToken = await this.issueClientToken(localUrl, this.configuredRokDesktopPassword(instance));
