...
import { mintOutsideFileGrant } from '@rok-desktop/server/server/lib/fs/routes.js';

const execFileAsync = promisify(execFile);

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const isDev = process.env.ROK_DESKTOP_ELECTRON_DEV === '1' || !app.isPackaged;

const DEEP_LINK_PROTOCOL = 'rok-desktop';
const UI_PROTOCOL = 'rok-desktop-ui';
const PACKAGED_APP_USER_MODEL_ID = 'ai.rokcode.desktop';
const DEV_APP_USER_MODEL_ID = 'ai.rokcode.desktop.dev';
...
// ~/Library/Logs/Rok Desktop/ (not ~/Library/Logs/@rok-desktop/electron/).
app.setName('Rok Desktop');
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'Rok Desktop Dev'));
}
...
// that output never lands in ~/Library/Logs/Rok Desktop/main.log and we
...
const readAppMetadata = () => {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (parsed?.name === '@rok-desktop/electron' && typeof parsed.version === 'string') {
        return { name: parsed.name, version: parsed.version };
      }
    } catch {
    }
  }
  return { name: '@rok-desktop/electron', version: app.getVersion() };
};
...
const CHANGELOG_URL = 'https://raw.githubusercontent.com/rokcode/rok-desktop/main/CHANGELOG.md';
const GITHUB_BUG_REPORT_URL = 'https://github.com/rokcode/rok-desktop/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/rokcode/rok-desktop/issues/new?template=feature_request.yml';
...
const quitConfirmationMessage = () => {
  ...
  return `Rok Desktop detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
const requestQuitWithConfirmation = async () => {
  ...
  try {
    const result = await dialog.showMessageBox({
      type: 'warning',
      title: 'Quit Rok Desktop?',
      message: 'Quit Rok Desktop?',
...
const refreshQuitRiskFlags = async () => {
  ...
  const scheduledUrl = `${base}/api/rok-desktop/scheduled-tasks/status`;
  const tunnelUrl = `${base}/api/rok-desktop/tunnel/status`;
...
const settingsFilePath = () => {
  if (typeof process.env.ROK_DESKTOP_DATA_DIR === 'string' && process.env.ROK_DESKTOP_DATA_DIR.trim()) {
    return path.join(process.env.ROK_DESKTOP_DATA_DIR.trim(), 'settings.json');
  }
  return path.join(os.homedir(), '.config', 'rok-desktop', 'settings.json');
};
...
const shouldUsePackagedUi = () => {
  if (process.env.ROK_DESKTOP_ELECTRON_LOAD_SERVER_UI === '1') return false;
  if (process.env.ROK_DESKTOP_ELECTRON_USE_BUNDLED_UI === '1') return true;
...
const injectRuntimeConfigIntoHtml = (html) => {
  ...
  const initScript = `<script>if(window.__ROK_DESKTOP_LOCAL_ORIGIN__===undefined){window.__ROK_DESKTOP_LOCAL_ORIGIN__=${JSON.stringify(localOrigin)};}if(window.__ROK_DESKTOP_API_BASE_URL__===undefined){window.__ROK_DESKTOP_API_BASE_URL__=${JSON.stringify(apiBaseUrl)};}if(window.__ROK_DESKTOP_CLIENT_TOKEN__===undefined&&${JSON.stringify(state.clientToken || '')}){window.__ROK_DESKTOP_CLIENT_TOKEN__=${JSON.stringify(state.clientToken || '')};}</script>`;
...
const maybeShowNativeNotification = (rawInput) => {
  ...
  notification.on('click', () => {
    ...
    if (sessionId) {
      emitToAllWindows('rok-desktop:open-session', { sessionId, directory });
    }
    release();
  });
...
import { pathLooksUserConfigured, mergePathValues } from '@rok-desktop/server/server/lib/opencode/path-utils.js';
...
const spawnLocalServer = async () => {
  ...
  // The server module reads ENV_DESKTOP_NOTIFY / ROK_DESKTOP_DIST_DIR /
  // ROK_DESKTOP_RUNTIME at import time (top-level const), so these must be
  ...
  process.env.ROK_DESKTOP_OPENCODE_CWD = resolveManagedOpenCodeCwd({
    env: process.env,
    homedir: () => os.homedir(),
  });
  ...
  if (desktopUiPassword) {
    ...
    delete process.env.ROK_DESKTOP_UI_PASSWORD;
  }
  process.env.ROK_DESKTOP_SKIP_API_COMPRESSION = process.env.ROK_DESKTOP_SKIP_API_COMPRESSION || 'true';
  process.env.NO_PROXY = process.env.NO_PROXY || 'localhost,127.0.0.1';
  process.env.no_proxy = process.env.no_proxy || 'localhost,127.0.0.1';

  const { startWebUiServer } = await import('@rok-desktop/server/server/index.js');
...
`;
  ...
  const child = spawn('/bin/sh', ['-c', script, 'rok-desktop-opencode-killer', normalizedPid, normalizedPort, String(OPENCODE_SHUTDOWN_GRACE_MS / 1000)], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
  });
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Rok Desktop loading icon">
...
const loginRemoteAndIssueClientToken = async ({ url, password, trustDevice, requestHeaders }) => {
  ...
  const loginResponse = await fetch(new URL('/auth/session', `${baseUrl}/`).toString(), {
    ...
    body: JSON.stringify({
      ...
      clientLabel: 'Rok Desktop Desktop',
...
const emitToWindow = (browserWindow, event, detail) => {
  if (!browserWindow || browserWindow.isDestroyed()) return;
  browserWindow.webContents.send('rok-desktop:emit', { event, detail });
};
...
const setMacVibrancyReady = (browserWindow, ready) => {
  if (process.platform !== 'darwin' || !browserWindow || browserWindow.isDestroyed()) return;
  emitToWindow(browserWindow, 'rok-desktop:vibrancy-ready', { ready });
};
...
const confirmConnectDeepLink = async (payload) => {
  ...
  const options = {
    type: 'warning',
    title: 'Connect to Rok Desktop server?',
...
const dispatchDeepLink = (link) => {
  ...
  if (link.type === 'session' && link.value) {
    emitToAllWindows('rok-desktop:open-session', { sessionId: link.value });
    return;
  }
  if (link.type === 'project' && link.value) {
    emitToAllWindows('rok-desktop:open-project', { projectPath: link.value });
    return;
  }
...
const dispatchMenuAction = (action) => {
  ...
  emitToWindow(target, 'rok-desktop:menu-action', action);
  dispatchDomEventToWindow(target, 'rok-desktop:menu-action', action);
};
...
const dispatchOpenMiniChat = (browserWindow) => {
  ...
  if (target) emitToWindow(target, 'rok-desktop:open-mini-chat');
};

const dispatchCheckForUpdates = () => {
  emitToAllWindows('rok-desktop:check-for-updates');
  for (const browserWindow of BrowserWindow.getAllWindows()) {
    dispatchDomEventToWindow(browserWindow, 'rok-desktop:check-for-updates');
  }
};
...
const createBrowserWindow = ({ label, restoreGeometry, url, runtimeConfig = {} }) => {
  ...
  const options = {
    ...
    webPreferences: {
      additionalArguments: [
        `--rok-desktop-local-origin=${desktopLocalOrigin}`,
        `--rok-desktop-api-base-url=${desktopApiBaseUrl}`,
        `--rok-desktop-client-token=${desktopClientToken}`,
        `--rok-desktop-runtime-headers=${JSON.stringify(desktopRequestHeaders)}`,
        `--rok-desktop-home=${desktopHome}`,
        `--rok-desktop-macos-major=${desktopMacosMajor}`,
        `--rok-desktop-mac-vibrancy=${useVibrancy ? '1' : '0'}`,
        `--rok-desktop-boot-outcome=${JSON.stringify(state.bootOutcome || null)}`,
      ],
  ...
  browserWindow.on('resize', () => {
    if (process.platform === 'darwin') {
      emitToWindow(browserWindow, 'rok-desktop:window-resized');
    }
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('maximize', () => {
    emitToWindow(browserWindow, 'rok-desktop:window-maximized-changed', { maximized: true });
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('unmaximize', () => {
    emitToWindow(browserWindow, 'rok-desktop:window-maximized-changed', { maximized: false });
    debounceWindowStatePersist(browserWindow, false);
  });
...
const resolveInitialUrl = async () => {
  const hmrApiPort = process.env.ROK_DESKTOP_HMR_API_PORT || '3901';
  const hmrUiPort = process.env.ROK_DESKTOP_HMR_UI_PORT || '5173';
...
const parseGithubRepo = () => {
  return { owner: 'rok-desktop', repo: 'rok-desktop' };
};
...
const setupAutoUpdater = () => {
  ...
  autoUpdater.on('download-progress', (progress) => {
    ...
    emitToAllWindows('rok-desktop:update-progress', mapUpdaterProgressEvent({
      event: 'Progress',
      data: {
...
const iconToDataUrl = async (iconPath, appName) => {
  ...
  const tempPath = path.join(os.tmpdir(), `rok-desktop-icon-${safeName}-${Date.now()}.png`);
  try {
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    case 'desktop_read_file': {
      ...
      const DENIED_SEGMENTS = ['.ssh', '.aws', '.gnupg', '.gpg', '.config/gh', '.config/rok-desktop/credentials'];
    ...
    case 'desktop_get_installed_apps': {
      ...
      const refresh = async () => {
        ...
        emitToAllWindows('rok-desktop:installed-apps-updated', apps);
      };
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires Rok Desktop.app to be installed in /Applications');
          }
        } catch (error) {
    ...
    case 'desktop_focus_main_window': {
      ...
      if (sessionId) {
        emitToWindow(state.mainWindow, 'rok-desktop:open-session', { sessionId, directory });
      } else if (mode === 'draft') {
        emitToWindow(state.mainWindow, 'rok-desktop:open-draft-session', { directory, projectId });
      }
      return { focused: true };
    }
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About Rok Desktop', click: () => dispatchAction('about') },
        {
...
ipcMain.handle('rok-desktop:invoke', async (event, command, args) => {
...
ipcMain.handle('rok-desktop:dialog:open', async (event, options) => {
...
ipcMain.handle('rok-desktop:file:grant-existing', async (event, filePath) => {
  if (!isLocalSender(event.sender)) {
...
const focusMainWindowWithSession = async (sessionId, directory) => {
  if (state.mainWindow && !state.mainWindow.isDestroyed()) {
    ...
    state.mainWindow.focus();
    if (sessionId) {
      emitToWindow(state.mainWindow, 'rok-desktop:open-session', { sessionId, directory: directory || '' });
    }
    return;
  }
  if (sessionId) pendingDeepLinks.push({ type: 'session', value: sessionId });
...
const dispatchTrayAction = async (action) => {
  ...
  if (action.type === 'respond-permission') {
    ...
    emitToWindow(target, 'rok-desktop:tray-action', action);
    return;
  }
  ...
  if (action.type === 'focus-session') {
    ...
    if (surface && surface.__ocMiniChat === true && action.sessionId) {
      ...
      surface.show();
      surface.focus();
      emitToWindow(surface, 'rok-desktop:open-session', {
  ...
  if (action.type === 'new-session') {
    emitToWindow(target, 'rok-desktop:open-draft-session', { directory: '', projectId: '' });
  }
...
app.whenReady().then(async () => {
  ...
  powerMonitor.on('resume', () => {
    emitToAllWindows('rok-desktop:system-resume', { timestamp: Date.now() });
  });
}).catch((error) => {
