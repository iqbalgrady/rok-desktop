...
const localOrigin = readArgValue('--rok-desktop-local-origin');
const apiBaseUrl = readArgValue('--rok-desktop-api-base-url');
const clientToken = readArgValue('--rok-desktop-client-token');
const runtimeHeadersRaw = readArgValue('--rok-desktop-runtime-headers');
const homeDirectory = readArgValue('--rok-desktop-home');
const macosMajorRaw = readArgValue('--rok-desktop-macos-major');
...
const hasMacVibrancy = macVibrancySupported && readArgValue('--rok-desktop-mac-vibrancy') !== '0';
...
//  - __ROK_DESKTOP_ELECTRON__ is a shell-identity flag (no capability).
...
//  - __ROK_DESKTOP_DESKTOP__ is the IPC channel to the main process. It is
...
const isLocalPage = currentOrigin !== 'null'
  && (currentOrigin === 'rok-desktop-ui://app'
  || (localOrigin && currentOrigin === localOrigin));

// Remote pages need __ROK_DESKTOP_LOCAL_ORIGIN__ so the HostSwitcher knows
...
if (localOrigin) {
  contextBridge.exposeInMainWorld('__ROK_DESKTOP_LOCAL_ORIGIN__', localOrigin);
}

if (apiBaseUrl) {
  contextBridge.exposeInMainWorld('__ROK_DESKTOP_API_BASE_URL__', apiBaseUrl);
}

if (clientToken && isLocalPage) {
  contextBridge.exposeInMainWorld('__ROK_DESKTOP_CLIENT_TOKEN__', clientToken);
}
...
if (runtimeHeadersRaw && isLocalPage) {
  try {
    ...
    if (runtimeHeaders && typeof runtimeHeaders === 'object') {
      contextBridge.exposeInMainWorld('__ROK_DESKTOP_RUNTIME_HEADERS__', runtimeHeaders);
    }
  } catch {
  }
}
...
if (isLocalPage && homeDirectory) {
  contextBridge.exposeInMainWorld('__ROK_DESKTOP_HOME__', homeDirectory);
}
...
if (Number.isFinite(macosMajor) && macosMajor > 0) {
  contextBridge.exposeInMainWorld('__ROK_DESKTOP_MACOS_MAJOR__', macosMajor);
}

contextBridge.exposeInMainWorld('__ROK_DESKTOP_ELECTRON__', {
  runtime: 'electron',
  macVibrancy: hasMacVibrancy,
  macVibrancySupported,
});

contextBridge.exposeInMainWorld('__ROK_DESKTOP_PLATFORM__', process.platform);
...
ipcRenderer.on('rok-desktop:emit', (_evt, payload) => {
  ...
  if (event === 'rok-desktop:vibrancy-ready') {
...
// ipcMain.handle('rok-desktop:invoke') decides per-command what is safe
// for non-local callers (window/host-switcher ops yes, file/shell ops
// no). See COMMANDS_SAFE_FOR_REMOTE in main.mjs.
contextBridge.exposeInMainWorld('__ROK_DESKTOP_DESKTOP__', {
  invoke: (cmd, args) => ipcRenderer.invoke('rok-desktop:invoke', cmd, args || {}),
  openDialog: (options) => ipcRenderer.invoke('rok-desktop:dialog:open', options || {}),
  grantFileAccess: (filePath) => ipcRenderer.invoke('rok-desktop:file:grant-existing', filePath),
  openExternal: (url) => ipcRenderer.invoke('rok-desktop:invoke', 'desktop_open_external_url', { url }),
