...
function readRokDesktopSettings(): Record<string, unknown> {
  const settingsPath = path.join(os.homedir(), '.config', 'rok-desktop', 'settings.json');
  try {
...
function createConfiguredOpencodeBinaryError(raw: string, normalized: string): Error {
  const messageSuffix = 'Rok Desktop needs the standalone opencode CLI. Install it and set rok-desktop.rokcodeBinary to the CLI path, for example ~/.opencode/bin/opencode, or leave the setting empty to use PATH lookup.';
...
function validateConfiguredOpencodeBinaryForManagedStart(): string | null {
  ...
  try {
    const settings = readRokDesktopSettings();
...
function resolveOpencodeCliPath(): string | null {
  ...
  if (sharedFromRok Desktop && isExecutable(sharedFromRok Desktop) && !isMacOpenCodeAppBundlePath(sharedFromRok Desktop)) {
    return sharedFromRok Desktop;
  }

  const explicit = [
    process.env.OPENCODE_BINARY,
    process.env.OPENCODE_PATH,
    process.env.ROK_DESKTOP_OPENCODE_PATH,
    process.env.ROK_DESKTOP_OPENCODE_BIN,
  ]
...
async function waitForReady(
  ...
  const outputChannel = vscode.window.createOutputChannel('RokDesktopManager');
