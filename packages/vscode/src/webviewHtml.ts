...
export function getWebviewHtml(options: WebviewHtmlOptions): string {
  ...
  // 
  // Logo geometry matches RokDesktopLogo.tsx:
...
<head>
  ...
  <title>Rok Desktop</title>
</head>
<body>
  <!-- Initial loading screen with simplified Rok Desktop logo -->
  ...
  <script>
    ...
    window.__ROK_DESKTOP_HOME__ = "${workspaceFolder.replace(/\\/g, '\\\\')}";
    
    function getBootstrapMessages() {
      var locale = 'en';
      try {
        var rawLocale = window.localStorage.getItem('rok-desktop.i18n.v1');
        if (rawLocale) {
  ...
  <script type="module">
    ...
    if (!devServerUrl) {
      ...
      const getDevMessages = () => {
        try {
          const rawLocale = window.localStorage.getItem('rok-desktop.i18n.v1');
          if (rawLocale) {
      ...
      const tryLoadDevBundle = () => {
        ...
        Promise.resolve()
          ...
          .catch((error) => {
            attempt += 1;
            console.warn('[Rok Desktop] VS Code webview dev bundle unavailable, retrying...', error);
