...
![VS Code Extension](https://github.com/rokcode/rok-desktop/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/rokcode/rok-desktop) with even more features.**
...
|---------|---------|-------------|
| `rok-desktop.apiUrl` | _(empty)_ | URL of an external OpenCode API server. Leave empty to auto-start a local instance. |
| `rok-desktop.rokcodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |
