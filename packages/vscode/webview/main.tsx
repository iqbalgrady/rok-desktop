...
import { opencodeClient } from '@rok-desktop/ui/lib/opencode/client';
import { sanitizeHeadersForBrowser } from '@rok-desktop/ui/lib/runtime-fetch';
import {
  buildVSCodeThemeFromPalette,
  readVSCodeThemePalette,
  type VSCodeThemeKind,
  type VSCodeThemePayload,
} from '@rok-desktop/ui/lib/theme/vscode/adapter';
import { getBootstrapMessages, readStoredLocaleForBootstrap } from '@rok-desktop/ui/lib/i18n';
import type { VSCodeActiveEditorFile } from '@/sync/input-store';

type ConnectionStatus = 'connecting' | 'connected' | 'error' | 'disconnected';
type PanelType = 'chat' | 'agentManager';

declare const __ROK_DESKTOP_WEBVIEW_BUILD_TIME__: string;
...
declare global {
  interface Window {
    ...
    __ROK_DESKTOP_VSCODE_THEME__?: VSCodeThemePayload['theme'];
    __ROK_DESKTOP_VSCODE_SHIKI_THEMES__?: { light?: Record<string, unknown>; dark?: Record<string, unknown> } | null;
    __ROK_DESKTOP_CONNECTION__?: { status: ConnectionStatus; error?: string; cliAvailable?: boolean };
    __ROK_DESKTOP_HOME__?: string;
    __ROK_DESKTOP_PANEL_TYPE__?: PanelType;
    __ROK_DESKTOP_VSCODE_WINDOW_FOCUSED__?: boolean;
  }
}

console.log('[Rok Desktop] VS Code webview starting...');
console.log('[Rok Desktop] VS Code webview build:', __ROK_DESKTOP_WEBVIEW_BUILD_TIME__);
console.log('[Rok Desktop] Config:', window.__VSCODE_CONFIG__);
try {
  if (window.localStorage.getItem('rok-desktop_stream_debug') === '1') {
    console.log('[Rok Desktop] Debug: rok-desktop_stream_debug=1');
  }
} catch {
  // ignore
}

window.__ROK_DESKTOP_RUNTIME_APIS__ = createVSCodeAPIs();
...
const bootstrapConnectionStatus = () => {
  ...
  window.__ROK_DESKTOP_CONNECTION__ = { status: initialStatus, cliAvailable };
};

bootstrapConnectionStatus();

// Expose panel type globally for the VS Code app root to conditionally render.
window.__ROK_DESKTOP_PANEL_TYPE__ = (window.__VSCODE_CONFIG__?.panelType as PanelType) || 'chat';
...
const handleConnectionMessage = (event: MessageEvent) => {
  ...
  if (msg?.type === 'connectionStatus') {
    ...
    const prevCliAvailable = window.__ROK_DESKTOP_CONNECTION__?.cliAvailable ?? true;
    window.__ROK_DESKTOP_CONNECTION__ = { status: payload, error, cliAvailable: prevCliAvailable };
    window.dispatchEvent(new CustomEvent('rok-desktop:connection-status', { detail: { status: payload, error } }));
  }
};

window.addEventListener('message', handleConnectionMessage);
window.addEventListener('rok-desktop:connection-status', () => {
...
const maybeHideLoadingOverlay = () => {
  const connectionStatus = window.__ROK_DESKTOP_CONNECTION__?.status ?? 'connecting';
  ...
  if (connectionStatus === 'error') {
    const error = window.__ROK_DESKTOP_CONNECTION__?.error;
...
const emitVSCodeTheme = (preferredKind?: VSCodeThemeKind) => {
  ...
  window.__ROK_DESKTOP_VSCODE_THEME__ = theme;
   applyInitialTheme(theme);
  window.dispatchEvent(new CustomEvent<VSCodeThemePayload>('rok-desktop:vscode-theme', {
...
onThemeChange((payload) => {
  ...
  if (typeof payload === 'object' && payload?.shikiThemes !== undefined) {
    window.__ROK_DESKTOP_VSCODE_SHIKI_THEMES__ = payload.shikiThemes;
    window.dispatchEvent(
      new CustomEvent('rok-desktop:vscode-shiki-themes', {
...
if (workspaceFolder) {
  ...
  window.__ROK_DESKTOP_HOME__ = normalizedWorkspaceFolder;
  try {
...
const handleLocalApiRequest = async (input: RequestInfo | URL, url: URL, init: RequestInit | undefined, method: string) => {
  ...
  if (normalizedPathname === '/api/system/info' && method === 'GET') {
    ...
    return jsonResponse({
      rokDesktopVersion: config?.extensionVersion || 'VS Code Extension',
      runtime: 'vscode',
  ...
  if (normalizedPathname.startsWith('/api/rok-desktop/tunnel/')) {
  ...
  if (pathname === '/health' || pathname === '/api/health') {
    const connectionStatus = window.__ROK_DESKTOP_CONNECTION__?.status;
    const isReady = connectionStatus === 'connected';
    const cliAvailable = window.__ROK_DESKTOP_CONNECTION__?.cliAvailable ?? true;
  ...
  if (pathname.startsWith('/api/rok-desktop/models-metadata')) {
    try {
      const data = await sendBridgeMessage('api:models/metadata');
      return new Response(JSON.stringify(data), { status: 200, headers: { 'Content-Type': 'application/json' } });
    } catch (error) {
      console.warn('[Rok Desktop] Failed to fetch models metadata via bridge, returning empty set:', error);
  ...
  if (pathname.startsWith('/api/rok-desktop/update-check')) {
    try {
      ...
      const data = await sendBridgeMessage('api:rok-desktop:update-check', {
        currentVersion,
        instanceMode,
        deviceClass,
        platform,
...
window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (targetUrl && targetUrl.hostname.includes('models.dev')) {
    try {
      ...
      console.warn('[Rok Desktop] models.dev request failed via bridge, returning empty metadata:', error);
      return new Response(JSON.stringify({}), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
  }
  ...
  Promise.all([
    ...
    if (currentProviderId && currentModelId) {
      ...
      sessionStore.sendMessage(
        ...
        console.error('[Rok Desktop] Failed to send prompt:', error);
      });
    } else {
...
onCommand('newSession', (payload) => {
  ...
  window.dispatchEvent(new CustomEvent('rok-desktop:navigate', { detail: { view: 'chat' } }));
});
...
onCommand('showSettings', () => {
  ...
  window.dispatchEvent(new CustomEvent('rok-desktop:navigate', { detail: { view: 'settings' } }));
});
...
onCommand('reloadOpenCode', () => {
  void import('@rok-desktop/ui/stores/useAgentsStore').then(({ reloadOpenCodeConfiguration }) => {
...
const claimRokDesktopNotification = async (payload: { title?: unknown; body?: unknown; sessionId?: unknown; tag?: unknown } | undefined): Promise<boolean> => {
...
const showRokDesktopNotification = (payload: { title?: unknown; body?: unknown; sessionId?: unknown; tag?: unknown; requireHidden?: unknown } | undefined) => {
  if (typeof Notification === 'undefined') {
    return false;
  }

  const show = async () => {
    const isVSCodeWindowFocused = window.__ROK_DESKTOP_VSCODE_WINDOW_FOCUSED__ ?? document.hasFocus();
    ...
    const sessionId = typeof payload?.sessionId === 'string' && payload.sessionId.trim().length > 0
      ...
      : '';
    if (!await claimRokDesktopNotification({ ...payload, title, body, sessionId })) {
      return false;
    }
...
onCommand('showNotification', (payload) => {
  showRokDesktopNotification(payload as { title?: unknown; body?: unknown; sessionId?: unknown; requireHidden?: unknown } | undefined);
});

onCommand('windowFocusChanged', (payload) => {
  if (typeof payload === 'object' && payload && typeof (payload as { focused?: unknown }).focused === 'boolean') {
    window.__ROK_DESKTOP_VSCODE_WINDOW_FOCUSED__ = (payload as { focused: boolean }).focused;
  }
});
...
const ensureNotificationSettingsSynced = async () => {
  if (!notificationSettingsSyncPromise) {
    notificationSettingsSyncPromise = import('@/lib/persistence')
      ...
      .catch((error) => {
        console.warn('[Rok Desktop] Failed to sync notification settings:', error);
      });
  }
...
window.addEventListener('rok-desktop:vscode-notification-event', (event) => {
...
onCommand('settingsSynced', () => {
  import('@rok-desktop/ui/lib/persistence').then(({ syncDesktopSettings }) => {
...
import('@rok-desktop/ui/apps/renderVSCodeApp')
  .then(async ({ renderVSCodeApp }) => {
    renderVSCodeApp(window.__ROK_DESKTOP_RUNTIME_APIS__ ?? createVSCodeAPIs());
    await waitForUiMount();
    uiMounted = true;
    maybeHideLoadingOverlay();
  })
  .catch((error) => {
    console.error('[Rok Desktop] Failed to bootstrap UI:', error);
