...
declare global {
  interface Window {
    __rokDesktopVsCodeStreamPerfState__?: StreamPerfState;
  }
}
...
const ensurePerfState = (): StreamPerfState | null => {
  ...
  if (!window.__rokDesktopVsCodeStreamPerfState__) {
    const startedAt = Date.now();
    window.__rokDesktopVsCodeStreamPerfState__ = {
  ...
  return window.__rokDesktopVsCodeStreamPerfState__;
};
