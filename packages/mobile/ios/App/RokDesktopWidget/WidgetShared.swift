...
enum WidgetStore {
    static let appGroup = "group.com.ecc.rokcode"
...
enum WidgetDeepLink {
    static func newSession() -> URL { URL(string: "rok-desktop://new")! }
    static func attention() -> URL { URL(string: "rok-desktop://sessions?filter=attention")! }
    static func status() -> URL { URL(string: "rok-desktop://status")! }
    static func settings() -> URL { URL(string: "rok-desktop://settings")! }
    static func changes() -> URL { URL(string: "rok-desktop://changes")! }
    static func files() -> URL { URL(string: "rok-desktop://view/files")! }
    static func instances() -> URL { URL(string: "rok-desktop://view/instances")! }
    static func session(_ id: String) -> URL {
        let encoded = id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? id
        return URL(string: "rok-desktop://session/\(encoded)") ?? newSession()
    }
}
...
// MARK: - Logo (full RokDesktop mark drawn from the SVG)

/// The RokDesktop logo, drawn to match packages/web/public/logo-dark-512x512.svg: an
