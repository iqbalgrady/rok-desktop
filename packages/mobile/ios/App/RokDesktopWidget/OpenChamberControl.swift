...
// Control Center control (iOS 18+): tap the RokDesktop logo to start a new session.
//
...
@available(iOS 18.0, *)
struct RokDesktopNewSessionControl: ControlWidget {
    var body: some ControlWidgetConfiguration {
        StaticControlConfiguration(kind: "RokDesktopNewSessionControl") {
        ...
        .description("Start a new RokDesktop session.")
    }
}

@available(iOS 18.0, *)
struct OpenNewSessionIntent: AppIntent {
    static let title: LocalizedStringResource = "New RokDesktop Session"
    static let openAppWhenRun: Bool = true
    static let isDiscoverable: Bool = true

    @MainActor
    func perform() async throws -> some IntentResult & OpensIntent {
        return .result(opensIntent: OpenURLIntent(URL(string: "rok-desktop://new")!))
    }
}
