...
- App id / package: `com.ecc.rokcode`; app name `OpenChamber`.
...
- **Deep links** — `rok-desktop://` URL scheme; a reusable intent vocabulary (`apps/deepLinks.ts`)
...
- **iOS widgets + Control Center + Notification Service Extension** — WidgetKit extension
  ...
  that refreshes widgets from push. All share the App Group `group.com.ecc.rokcode`.
...
- App Group `group.com.ecc.rokcode` in all three targets' entitlements (app + widget + NSE).
- `Info.plist`: `CFBundleURLTypes` scheme `rok-desktop`, `NSCameraUsageDescription`.
...
- Apple Developer account; App IDs for the app **and** both extensions
  (`com.ecc.rokcode`, `.OpenChamberWidget`, `.OpenChamberNotificationService`), each enabled for
