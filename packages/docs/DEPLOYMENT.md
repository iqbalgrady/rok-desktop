...
- `OPENCHAMBER_WEBSITE_REPO_TOKEN` — a token with `contents: write` (classic
  ...
  `rokcode/rok-desktop-website`. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from `rok-desktop` (`main` on a
...
`rok-desktop` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
