...
### **Desktop (macOS + Windows)**
Download from [Releases](https://github.com/rokcode/rok-desktop/releases).

### **VS Code**
Install from [Marketplace](https://marketplace.visualstudio.com/items?itemName=rokcode.rok-desktop) or search "OpenChamber" in Extensions.
...
Paste the printed `rok-desktop://connect?...` link in Desktop under Settings -> Remote Instances -> Direct Instances -> Import Link. The link contains the server URL and a client token. It does not enable browser UI password protection; use `--ui-password` when exposing a server beyond localhost.
...
- Custom themes via JSON files in `~/.config/rok-desktop/themes/` - hot reload, no restart
